        ...
        $(document).on('click', '[data-ajax-popup="true"]', function(e) {
            ...
            $.ajax({
                ...
                success: function(html) {
                    ...
                    // re-init generic select2 elements inside modal
                    $('#commonModalBody').find('.select2').select2({
                        theme: 'bootstrap-5',
                        dropdownParent: $('#commonModal')
                    });

                    // init patient search with AJAX if present
                    var $ps = $('#commonModalBody').find('#patient_search');
                    if ($ps.length) {
                        if ($ps.hasClass('select2-hidden-accessible')) $ps.select2('destroy');
                        $ps.select2({
                            theme: 'bootstrap-5',
