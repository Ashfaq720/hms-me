...
<aside class="app-menubar" id="appMenubar">
    ...
    <nav class="app-navbar" data-simplebar>
        <ul class="menubar">
            ...
            {{-- ───────── COMMAND CENTERS (hidden — uncomment to restore) ─────────
            ...
            @php
                ...
                try {
                    $u = auth()->user();
                    $isDoctor = $u && method_exists($u, 'doctor') && $u->doctor()->exists();
                } catch (\Throwable $e) { $isDoctor = false; }
            @endphp
            @if ($isDoctor && Route::has('doctor-portal.index'))
            <li class="menu-item">
            ...
            <li class="menu-item menu-arrow{{ $otActive ? ' route-active' : '' }}">
                ...
                <ul class="menu-inner">
                    ...
                    @endcan
                    {{-- OT Master moved to Configuration → Master Data Setup → OT Master.
                         Operational OT items (Surgery Request, Scheduling, Intra-Op, …) stay here. --}}
                </ul>
            </li>
            @endcanany
            ...
            @php
                ...
                $onPathology        = request()->routeIs('pathology.index');
                $onRadiology        = request()->routeIs('radiology.index');
                $diagnosticsActive  = $onPathology || $onRadiology;
                $allOrdersActive    = $onPathology && $currentType === '';
            @endphp
            <li class="menu-item menu-arrow{{ $diagnosticsActive ? ' route-active' : '' }}">
                {{-- Parent anchor: do NOT add .active here. The strong highlight
                     would collide with the matching child below and look like
                     multiple items are selected. Use only .route-active on the
                     <li> so the submenu stays expanded. --}}
                ...
                <ul class="menu-inner">
                    <li class="menu-item">
                        <a class="menu-link{{ $allOrdersActive ? ' active' : '' }}"
                    ...
                    @endphp
                    @foreach ($diagTypes as [$label, $diagRoute, $icon, $colour])
                        @php
                            // Decide href + active per row. Reset every iteration
                            // so nothing leaks between loops.
                            $isActive = false;
                            $href     = route('pathology.index') . '?type=' . urlencode($label);

                            if ($diagRoute === 'radiology.index' && \Illuminate\Support\Facades\Route::has('radiology.index')) {
                                $href     = route('radiology.index');
                                $isActive = $onRadiology;
                            } else {
                                // Everything else (Pathology, Microbiology, …) lives under /pathology
                                // with the appropriate ?type=… filter.
                                $isActive = $onPathology && $currentType === $label;
                            }
                        @endphp
                        <li class="menu-item">
                    ...
                    @endforeach
                    {{-- Lab Investigation master data lives under
                         "Configuration → Master Data Setup → Lab Investigation".
                         Not duplicated here. --}}
                </ul>
            </li>

            <li class="menu-item menu-arrow{{ isMultiRoute(['admin.medicines.', 'admin.pharmacy.']) ? ' route-active' : '' }}">
                ...
                <ul class="menu-inner">
                    ...
                    {{-- Pharmacy master data lives under "Configuration → Master Data Setup → Pharmacy Master".
            ...
            {{-- ===== AMBULANCE MODULE ===== --}}
            <li class="menu-item menu-arrow{{ request()->routeIs('amb.requests.*', 'amb.trips.*', 'amb.er.incoming') ? ' route-active' : '' }}">
                <a class="menu-link{{ request()->routeIs('amb.requests.*', 'amb.trips.*', 'amb.er.incoming') ? ' active' : '' }}"
                    href="javascript:void(0);" role="button">
                ...
                <ul class="menu-inner">
                    <li class="menu-item">
                        <a class="menu-link{{ request()->routeIs('amb.requests.*') && !request()->routeIs('amb.requests.create') ? ' active' : '' }}"
                    ...
                    <li class="menu-item">
                        <a class="menu-link{{ request()->routeIs('amb.trips.*') ? ' active' : '' }}"
                            href="{{ route('amb.requests.index') }}"
                            title="Open a Request record to dispatch an ambulance">
                            <i class="bi bi-geo-alt-fill"></i>
                            <span class="menu-label">Dispatch &amp; Trips</span>
                        </a>
                    </li>
                    {{-- "Vendors" master moved to Configuration → Master Data Setup → Ambulance Setup --}}
                </ul>
            </li>
            <li class="menu-item menu-arrow{{ request()->routeIs('er.*', 'amb.er.*') ? ' route-active' : '' }}">
                ...
                <ul class="menu-inner">
                    ...
                    {{-- "Emergency Billing" lives under Billing (Legacy). Not duplicated here. --}}
                </ul>
            </li>
            <li class="menu-item">
            ...
            @php
                $masterDataSections = [
                    'patients.', 'doctors.', 'floor.', 'bed-groups.', 'bed-types.', 'beds.',
                    'designations.', 'departments.', 'specialists.', 'doctor-fees.',
                    'packages.', 'services.',
                    'lab-investigation-types.', 'lab-investigation-categories.', 'lab-investigations.',
                    'symptoms.',
                    'bb.blood-groups.', 'bb.components.', 'bb.temperature-rules.', 'bb.storage-locations.',
                    'bb.blood-bags.', 'bb.deferral-reasons.', 'bb.blood-donors.',
                    'appointment-priorities.', 'shifts.', 'doctor-shifts.', 'doctor-slots.',
                    'amb.ambulances.', 'amb.drivers.', 'amb.paramedics.', 'amb.vendors.',
                    // OT master routes (moved into MDS):
                    'ot.setup.', 'ot.rooms.', 'ot.teams.', 'ot.equipments.',
                    // Inventory master routes (moved into MDS):
                    'inventory.items.', 'inventory.warehouses.',
                    'admin.charges.', 'admin.unite-types.', 'admin.tax-categories.',
                    'admin.charge-types.', 'admin.charge-categories.',
                    'admin.medicine-categories.', 'admin.medicine-units.', 'admin.medicine-generics.',
                    'admin.companies.', 'admin.suppliers.', 'admin.medical-groups.', 'admin.medicines.',
                ];
                $masterDataActive = isMultiRoute($masterDataSections);
            @endphp
            <li class="menu-item menu-arrow{{ $masterDataActive ? ' route-active' : '' }}">
                <a class="menu-link{{ $masterDataActive ? ' active' : '' }}"
                    href="javascript:void(0);" role="button">
                ...
                <ul class="menu-inner">
                    ...
                    <li class="menu-item">
                        <a class="menu-link {{ request()->routeIs('admin.charges.*', 'admin.unite-types.*', 'admin.tax-categories.*', 'admin.charge-types.*', 'admin.charge-categories.*') ? 'active' : '' }}"
                    ...
                    --}}
                    <li class="menu-item menu-arrow{{ request()->routeIs('amb.ambulances.*', 'amb.drivers.*', 'amb.paramedics.*', 'amb.vendors.*') ? ' route-active' : '' }}">
                        <a class="menu-link{{ request()->routeIs('amb.ambulances.*', 'amb.drivers.*', 'amb.paramedics.*', 'amb.vendors.*') ? ' active' : '' }}"
                            href="javascript:void(0);" role="button">
                            <i class="fas fa-ambulance"></i>
                            <span class="menu-label">Ambulance Setup</span>
                        </a>
                        <ul class="menu-inner">
                            <li class="menu-item">
                                <a class="menu-link{{ request()->routeIs('amb.ambulances.*') ? ' active' : '' }}"
                                    href="{{ route('amb.ambulances.index') }}">
                                    <i class="bi bi-truck-front"></i>
                                    <span class="menu-label">Fleet / Vehicles</span>
                                </a>
                            </li>
                            <li class="menu-item">
                                <a class="menu-link{{ request()->routeIs('amb.drivers.*') ? ' active' : '' }}"
                            ...
                            <li class="menu-item">
                                <a class="menu-link{{ request()->routeIs('amb.paramedics.*') ? ' active' : '' }}"
                                    href="{{ route('amb.paramedics.index') }}">
                                    <i class="fas fa-user-nurse"></i>
                                    <span class="menu-label">Paramedics</span>
                                </a>
                            </li>
                            <li class="menu-item">
                                <a class="menu-link{{ request()->routeIs('amb.vendors.*') ? ' active' : '' }}"
                                    href="{{ route('amb.vendors.index') }}">
                    ...
                    <li class="menu-item menu-arrow{{ request()->routeIs('packages.*') ? ' route-active' : '' }}">
                        ...
                        <ul class="menu-inner">
                            ...
                            @foreach ($typeFilters as $typeCode => [$icon, $label])
                                <li class="menu-item">
                                    <a class="menu-link{{ request()->routeIs('packages.index') && request('type') === $typeCode ? ' active' : '' }}"
                    ...
                    {{-- ── OT Master (moved out of "OT Management") ── --}}
                    @can('ot_setup_access')
                        @php
                            $otMasterActive = request()->routeIs('ot.setup.*', 'ot.rooms.*', 'ot.teams.*', 'ot.equipments.*');
                        @endphp
                        <li><hr class="dropdown-divider my-1"></li>
                        <li class="menu-heading px-3 small text-muted">OT Master</li>
                        <li class="menu-item">
                            <a class="menu-link{{ request()->routeIs('ot.setup.index') ? ' active' : '' }}"
                                href="{{ route('ot.setup.index') }}">
                                <i class="bi bi-sliders"></i><span class="menu-label">Setup Overview</span>
                            </a>
                        </li>
                        @can('ot_room_access')
                            <li class="menu-item">
                                <a class="menu-link{{ request()->routeIs('ot.rooms.*') || request()->routeIs('ot.setup.rooms.*') ? ' active' : '' }}"
                                    href="{{ route('ot.rooms.index') }}">
                                    <i class="bi bi-door-closed"></i><span class="menu-label">OT Rooms</span>
                                </a>
                            </li>
                        @endcan
                        {{-- "OT Equipment" lives under Inventory & Stock → Live Equipment by Unit. --}}
                        @can('ot_team_access')
                            <li class="menu-item">
                                <a class="menu-link{{ request()->routeIs('ot.teams.*') ? ' active' : '' }}"
                                    href="{{ route('ot.teams.index') }}">
                                    <i class="bi bi-people"></i><span class="menu-label">OT Teams</span>
                                </a>
                            </li>
                        @endcan
                        <li class="menu-item">
                            <a class="menu-link{{ request()->routeIs('ot.setup.surgery-types.*') ? ' active' : '' }}"
                                href="{{ route('ot.setup.surgery-types.index') }}">
                                <i class="bi bi-clipboard"></i><span class="menu-label">Surgery Types</span>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a class="menu-link{{ request()->routeIs('ot.setup.surgery-categories.*') ? ' active' : '' }}"
                                href="{{ route('ot.setup.surgery-categories.index') }}">
                                <i class="bi bi-tags"></i><span class="menu-label">Surgery Categories</span>
                            </a>
                        </li>
                        <li class="menu-item">
                            <a class="menu-link{{ request()->routeIs('ot.setup.anesthesia-types.*') ? ' active' : '' }}"
                                href="{{ route('ot.setup.anesthesia-types.index') }}">
                        ...
                        <li class="menu-item">
                            <a class="menu-link{{ request()->routeIs('ot.setup.consumables.*') ? ' active' : '' }}"
                                ...
                                <i class="bi bi-box-seam"></i><span class="menu-label">OT Consumables</span>
                            </a>
                        </li>
                    @endcan

                    {{-- ── Inventory Master (moved out of "Inventory & Stock") ── --}}
                    @canany(['inventory.item.view', 'inventory.warehouse.view'])
                        @php
                            $inventoryMasterActive = request()->routeIs('inventory.items.*')
                                || request()->routeIs('inventory.warehouses.*');
                        @endphp
                        <li><hr class="dropdown-divider my-1"></li>
                        <li class="menu-heading px-3 small text-muted">Inventory Master</li>
                        @can('inventory.item.view')
                            <li class="menu-item">
                                <a class="menu-link{{ request()->routeIs('inventory.items.*') && !request('type') ? ' active' : '' }}"
                            ...
                            <li class="menu-item">
                                <a class="menu-link{{ request()->routeIs('inventory.items.*') && request('type') === 'medicine' ? ' active' : '' }}"
                            ...
                            <li class="menu-item">
                                <a class="menu-link{{ request()->routeIs('inventory.items.*') && request('type') === 'consumable' ? ' active' : '' }}"
                            ...
                            <li class="menu-item">
                                <a class="menu-link{{ request()->routeIs('inventory.items.*') && request('type') === 'asset' ? ' active' : '' }}"
            ...
            @canany(['inventory.item.view', 'inventory.warehouse.view', 'inventory.stock.view'])
                ...
                <li class="menu-item menu-arrow{{ $invActive ? ' route-active' : '' }}">
                    ...
                    <ul class="menu-inner">
                        ...
                        {{-- Items + Warehouses moved to Configuration → Master Data Setup → Inventory Master --}}
                        ...
                        <li><hr class="dropdown-divider my-1"></li>
                        <li class="menu-heading px-3 small text-muted">Live Equipment by Unit</li>
                        {{-- All unit-level equipment in one place: OT / ICU / CCU / NICU.
                             The pharmacy-style OT Master in MDS lists Setup Overview,
                             Rooms, Teams, Surgery Types, Categories, Anesthesia Types,
                             Consumables — but NOT equipment (that's here). --}}
                        @can('ot_setup_access')
                            <li class="menu-item">
