            ...
            <table class="table table-hover align-middle mb-0" id="dt_basic">
                <thead class="table-light">
                    <tr>
                        ...
                        <th class="py-3 text-muted small fw-semibold">LIVE STOCK</th>
                ...
                <tbody>
                    @forelse($medicines as $medicine)
                        <tr>
                            <td class="px-3 py-3">
                                {{-- Canonical code lives on inventory_items so the same medicine has
                                     ONE code across Drug Master, Inventory, prescriptions and barcodes.
                                     Falls back to a synthetic value only if the inventory linkage is
                                     missing (shouldn't happen after UnifyMedicineInventorySeeder). --}}
                                <span class="text-primary fw-medium">
                                    {{ $medicine->inventoryItem->code ?? 'MED-' . str_pad($medicine->id, 5, '0', STR_PAD_LEFT) }}
                                </span>
                            </td>
                            <td class="py-3">
                            ...
                            <td class="py-3">
                                @php
                                    $stock = (int) $medicine->live_stock;
                                    $reorder = is_numeric($medicine->reorder_level ?? null) ? (int) $medicine->reorder_level : 0;
                                @endphp
                                @if($stock <= 0)
                                    <span class="badge bg-danger bg-opacity-10 text-danger rounded-pill" title="Across all warehouses">{{ $stock }} — Out</span>
                                @elseif($reorder > 0 && $stock <= $reorder)
                                    <span class="badge bg-warning bg-opacity-10 text-warning rounded-pill" title="Across all warehouses">{{ $stock }} — Low</span>
                                @else
                                    <span class="fw-medium" title="Across all warehouses">{{ $stock }}</span>
                                @endif
                                <small class="text-muted d-block">{{ $medicine->unit->name ?? '' }}</small>
                            </td>
                            <td class="py-3">
                                @if($medicine->status)
