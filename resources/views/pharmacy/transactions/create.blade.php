<form action="{{ route('admin.pharmacy.transactions.store') }}" method="POST" id="txnForm" autocomplete="off">
    @csrf
    ...
    <div id="section_opd" class="txn-section">
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-6">
                ...
                <select name="opd_patient_id" id="opd_patient_id" class="form-select"
                        data-placeholder="Select OPD Patient">
                    <option value=""></option>
                    @foreach($opdPatients as $op)
                        <option value="{{ $op->id }}"
                                data-case="{{ $op->case_id }}"
                                data-pid="{{ $op->patient_id }}"
                                data-mrn="{{ $op->patient->mrn ?? '' }}"
                                data-phone="{{ $op->patient->mobileno ?? '' }}"
                                data-pname="{{ $op->patient->patient_name ?? 'Unknown' }}">
                            {{ $op->patient->patient_name ?? 'Unknown' }} — MRN {{ $op->patient->mrn ?? '-' }} (case {{ $op->case_id }})
                        </option>
                    @endforeach
                </select>
                <small id="opd_patient_info" class="text-muted d-block mt-1" style="min-height:1.2em;"></small>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-3">
                ...
                <select name="prescription_id" class="form-select"
                        data-placeholder="Select Rx (optional)">
                    <option value=""></option>
    ...
    <div id="section_ipd" class="txn-section d-none">
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-6">
                ...
                <select name="ipd_patient_id" id="ipd_patient_id" class="form-select"
                        data-placeholder="Select Admitted Patient">
                    <option value=""></option>
                    @foreach($ipdPatients as $ip)
                        <option value="{{ $ip->id }}"
                                data-pid="{{ $ip->patient_id }}"
                                data-ipdno="{{ $ip->ipd_no ?? $ip->id }}"
                                data-mrn="{{ $ip->patient->mrn ?? '' }}"
                                data-phone="{{ $ip->patient->mobileno ?? '' }}"
                                data-pname="{{ $ip->patient->patient_name ?? 'Unknown' }}">
                            {{ $ip->patient->patient_name ?? 'Unknown' }} — MRN {{ $ip->patient->mrn ?? '-' }} (IPD {{ $ip->ipd_no ?? $ip->id }})
                        </option>
                    @endforeach
                </select>
                <small id="ipd_patient_info" class="text-muted d-block mt-1" style="min-height:1.2em;"></small>
            </div>
            ...
            <div class="col-md-6">
                ...
                <div class="d-flex gap-2">
                    <div style="flex:1 1 0;min-width:0;">
                        <select id="ipd_medicine_order" class="form-select" style="width:100%;"
                                data-placeholder="Select patient first">
                            <option value=""></option>
                        </select>
                    </div>
            ...
            <div class="col-md-4">
                ...
                <select name="request_source" class="form-select"
                ...
                <tr class="medicine-row" data-row="0">
                    <td class="py-2">
                        <select name="items[0][medicine_id]"
                                ...
                                data-placeholder="Select Drug" required>
                            <option value=""></option>
                    ...
                    <td class="py-2">
                        <select name="items[0][store]"
                                class="form-select form-select-sm store-select"
...
(function () {
    ...
    function initSelect2(el, placeholder) {
        ...
        $el.select2(opts);
        // Select2 creates <span class="select2 select2-container ..."> next to the <select>.
        // The global initSelect2 (main.js), fired on shown.bs.modal, matches ".select2" and
        // would try to re-init these container spans (they lack select2-hidden-accessible),
        // producing duplicate boxes. Stripping the "select2" class prevents that.
        $el.next('.select2-container').removeClass('select2');
    }
    ...
    function filterPrescriptions(opdPatientId) {
        ...
        $rxSel.empty().append('<option value=""></option>');
    ...
    function populateIpdOrders(ipdPatientId) {
        ...
        if (!ipdPatientId) {
            ...
            $sel.attr('data-placeholder', 'Select patient first');
        ...
        if (!list.length) {
            $sel.append('<option value=""></option>');
            $sel.attr('data-placeholder', 'No pending orders for this patient');
        ...
        $sel.append('<option value=""></option>');
        $sel.attr('data-placeholder', '— select an order —');
        list.forEach(o => {
    ...
    document.querySelectorAll('.txn-type-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            const $shownSection = document.getElementById('section_' + t);
            $shownSection.classList.remove('d-none');

            // The Select2 widgets inside a section that started with d-none were
            // measured at 0×0 width. Re-init them now the section is visible so
            // their dropdown panels position correctly.
            if (typeof $ !== 'undefined' && $.fn.select2) {
                $($shownSection).find('select').each(function () {
                    const $sel = $(this);
    ...
    function buildRow(idx) {
        // Empty first option lets Select2 render its data-placeholder attribute
        // instead of duplicating "Select Drug" both inside the widget AND inside
        // the dropdown list.
        let medOpts = '<option value=""></option>';
        medicines.forEach(m => { medOpts += `<option value="${m.id}">${m.name}</option>`; });
        let storeOpts = '<option value=""></option>';
    ...
    document.getElementById('addMedicineBtn').onclick = function () {
        const tr = buildRow(rowIndex++);
        document.getElementById('medicineRows').appendChild(tr);
        // Both selects on the new row need Select2; otherwise the Store column
        // stays as a native dropdown and looks visually mismatched with the
        // existing row.
        initSelect2(tr.querySelector('.medicine-select'), 'Select Drug');
        initSelect2(tr.querySelector('.store-select'),   'Select Store');
    };

    // Select2 events (jQuery delegated — fires instead of / in addition to native change)
    if (typeof $ !== 'undefined' && $.fn.select2) {
        // Patient select → fill OPD case no + filter prescriptions + show patient info line
        $('#commonModalBody')
            ...
            .on('select2:select select2:clear', '#opd_patient_id', function () {
                ...
                document.getElementById('opd_case_display').value = $opt.data('case') || '';

                const info = document.getElementById('opd_patient_info');
                if ($sel.val() && info) {
                    info.innerHTML = '<i class="bi bi-person-fill text-primary me-1"></i>' +
                        ($opt.data('pname') || '—') +
                        ' &nbsp;|&nbsp; MRN: <strong>' + ($opt.data('mrn') || '—') + '</strong>' +
                        ' &nbsp;|&nbsp; <i class="bi bi-telephone"></i> ' + ($opt.data('phone') || '—');
                } else if (info) {
                    info.textContent = '';
                }

                filterPrescriptions($sel.val());
            });
        ...
        // Ipd patient select → fetch medicine orders + show patient info line
        $('#commonModalBody')
            ...
            .on('select2:select select2:clear', '#ipd_patient_id', function () {
                ...
                const info = document.getElementById('ipd_patient_info');
                if ($sel.val() && info) {
                    info.innerHTML = '<i class="bi bi-person-fill text-info me-1"></i>' +
                        ($opt.data('pname') || '—') +
                        ' &nbsp;|&nbsp; MRN: <strong>' + ($opt.data('mrn') || '—') + '</strong>' +
                        ' &nbsp;|&nbsp; IPD: <strong>' + ($opt.data('ipdno') || '—') + '</strong>' +
                        ' &nbsp;|&nbsp; <i class="bi bi-telephone"></i> ' + ($opt.data('phone') || '—');
                } else if (info) {
                    info.textContent = '';
                }

                populateIpdOrders($sel.val());
            });
    ...
    // ─────────────────────────────────────────────────
    // Initial Select2 init — runs when the form first appears
    // (covers both standalone page AND modal-loaded contexts).
    // Without this the dropdowns stay as native selects and look "empty"
    // because the Bootstrap modal CSS expects the Select2 widget to render.
    // ─────────────────────────────────────────────────
    function initAllSelect2OnceReady() {
        if (typeof $ === 'undefined' || !$.fn.select2) {
            // jQuery / Select2 not loaded yet — retry once after the modal scripts finish
            setTimeout(initAllSelect2OnceReady, 200);
            return;
        }
        const targets = [
            { sel: '#opd_patient_id',              placeholder: 'Select OPD Patient' },
            { sel: '#section_opd [name="prescription_id"]', placeholder: 'Select Rx (optional)' },
            { sel: '#ipd_patient_id',              placeholder: 'Select Admitted Patient' },
            { sel: '#ipd_medicine_order',          placeholder: 'Select patient first' },
            { sel: '[name="request_source"]',      placeholder: 'Select Staff' },
            { sel: '#medicineRows .medicine-select', placeholder: 'Select Drug' },
            { sel: '#medicineRows .store-select',  placeholder: 'Select Store' },
        ];
        targets.forEach(t => {
            document.querySelectorAll(t.sel).forEach(el => {
                // Skip selects in hidden sections — they get initialized when
                // the tab switch handler shows their section.
                if (el.closest('.txn-section.d-none')) return;
                initSelect2(el, t.placeholder);
            });
        });
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initAllSelect2OnceReady);
    } else {
        initAllSelect2OnceReady();
    }
})();
</script>
