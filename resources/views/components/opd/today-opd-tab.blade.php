            ...
            <tbody>
                ...
                {{-- @dd($patient) --}}
                    <tr>
                        ...
                        @php
                            $chargeRow  = isset($chargesByEncounter) ? $chargesByEncounter->get($patient->encounter_id) : null;
                            $bill       = isset($billsByEncounter)   ? $billsByEncounter->get($patient->encounter_id)   : null;
                            $chargeTot  = $chargeRow ? (float) $chargeRow->charge_total : 0;
                            $chargePost = $chargeRow ? (int)   $chargeRow->posts        : 0;
                        @endphp
                        <td class="text-end">
                            @if ($chargePost)
                                <strong>৳ {{ number_format($chargeTot, 2) }}</strong>
                                <br><small class="text-muted">{{ $chargePost }} line(s)</small>
                            @else
                        ...
                        <td class="text-end">
                            @if ($bill)
                                <a href="{{ route('billing.bills.show', $bill) }}" class="text-decoration-none">
                                    <strong>৳ {{ number_format((float) $bill->grand_total, 2) }}</strong>
                                </a>
                                <br>
                                @if ($bill->balance_due > 0.01)
                                    <small class="text-danger">Due ৳ {{ number_format((float) $bill->balance_due, 2) }}</small>
                                @else
