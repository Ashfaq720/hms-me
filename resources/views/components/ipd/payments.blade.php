@php
    // Merge legacy transactions with the new unified bill payments so the
    // tab shows ALL money received — historical (transactions) + modern (bill_payments).
    $legacy = $iPDPatient->transactions()->latest()->get();

    $unifiedPayments = collect();
    if ($iPDPatient->encounter_id) {
        ...
        foreach ($bills as $bill) {
            foreach ($bill->payments as $p) {
                // Mirror the legacy `transactions` schema so the existing
                // row template + detail modal don't blow up on missing properties.
                $unifiedPayments->push((object) [
                    'id'                   => 'bp-' . $p->id,
                    'patient_id'           => $bill->patient_id,
                    'case_id'              => null,
                    'ipd_patient_id'       => $iPDPatient->id,
                    'opd_patient_id'       => null,
                    'pathology_bill_id'    => null,
                    'pharmacy_bill_id'     => null,
                    'radiology_bill_id'    => null,
                    'blood_bank_bill_id'   => null,
                    ...
                    'notes'                => $p->notes,
                    'received_by'          => $p->collected_by,
                    'files'                => null,
                    'status'               => $p->status,
                    'created_at'           => $p->created_at,
                    'updated_at'           => $p->updated_at,
                    // extras for our own use
                    'is_auto'              => true,
                    'bill_id'              => $bill->id,
    ...
    $payments = $legacy->concat($unifiedPayments);
    $totalLegacy   = (float) $legacy->sum('net_amount');
    $totalUnified  = (float) $unifiedPayments->sum('amount');
    $totalAll      = (float) $payments->sum('net_amount');
@endphp
    ...
    <div class="col-md col-6">
        <div class="card border-0 shadow-sm bg-success bg-opacity-10">
            <div class="card-body py-2 px-3">
                <small class="text-success"><i class="bi bi-cash-coin"></i> Total Paid</small>
                <h5 class="mb-0">৳ {{ number_format($totalAll, 0) }}</h5>
                <small class="text-muted">{{ $payments->count() }} payments</small>
            </div>
        </div>
    </div>
    <div class="col-md col-6">
        <div class="card border-0 shadow-sm bg-primary bg-opacity-10">
            <div class="card-body py-2 px-3">
                <small class="text-primary"><i class="bi bi-credit-card-2-front"></i> Bill Payments</small>
                <h5 class="mb-0">৳ {{ number_format($totalUnified, 0) }}</h5>
                <small class="text-muted">{{ $unifiedPayments->count() }} encounter-linked</small>
            </div>
        </div>
    </div>
    <div class="col-md col-6">
        <div class="card border-0 shadow-sm bg-warning bg-opacity-10">
            <div class="card-body py-2 px-3">
                <small class="text-warning"><i class="bi bi-receipt"></i> Manual / Legacy</small>
                <h5 class="mb-0">৳ {{ number_format($totalLegacy, 0) }}</h5>
                <small class="text-muted">{{ $legacy->count() }} entries</small>
            </div>
        </div>
    </div>
</div>
...
<div class="table-responsive">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h6 class="mb-0">Patient Payments <small class="text-muted">(merged: legacy + bill payments)</small></h6>
        </div>
        <div>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <div class="d-flex gap-1 justify-content-center">
                            ...
                            @if (! empty($payment->is_auto))
                                {{-- Unified bill payment (read-only from this tab). Link to the bill. --}}
                                <a href="{{ route('billing.bills.show', $payment->bill_id) }}"
                                    class="btn btn-sm btn-outline-success" title="Open bill"
                                    target="_blank">
                                    <i class="bi bi-receipt"></i>
                                </a>
                                <span class="btn btn-sm btn-light disabled" title="Manage via Bills"
                                    style="pointer-events:none;cursor:not-allowed;opacity:.5">
