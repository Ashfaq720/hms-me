...
<div class="p-2" style="font-size: 0.82rem;">
    ...
    @php
        // Detect NICU admission linked to this IPD (baby case)
        $nicuRecord = \App\Models\Nicu\NicuAdmission::with(['mother', 'motherIpd', 'resources'])
        ...
        <div class="card border-warning bg-warning bg-opacity-10 mb-3">
            <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h6 class="mb-2"><i class="bi bi-emoji-smile text-warning"></i>
                            NICU Patient · Baby ID <strong>{{ $nicuRecord->baby_id }}</strong>
                        </h6>
                        <div class="d-flex flex-wrap gap-2 small mb-2">
                            <span class="badge bg-secondary">Source: {{ $nicuRecord->source }}</span>
                            <span class="badge bg-info">Birth: {{ $nicuRecord->birth_type }}</span>
                            @if ($nicuRecord->birth_weight_g)
                                <span class="badge bg-light text-dark">Birth wt: {{ number_format($nicuRecord->birth_weight_g, 0) }}g</span>
                            @endif
                            @if ($nicuRecord->gestational_age_weeks)
                                <span class="badge bg-light text-dark">GA: {{ $nicuRecord->gestational_age_weeks }}w</span>
                            @endif
                            <span class="badge bg-light text-dark">APGAR: {{ $nicuRecord->apgar_1min ?? '?' }}/{{ $nicuRecord->apgar_5min ?? '?' }}/{{ $nicuRecord->apgar_10min ?? '?' }}</span>
                            @if ($nicuRecord->is_critical)<span class="badge bg-danger">Critical</span>@endif
                            @if ($nicuRecord->is_preterm)<span class="badge bg-warning text-dark">Preterm</span>@endif
                            @if ($nicuRecord->is_low_birth_weight)<span class="badge bg-warning text-dark">LBW</span>@endif
                        </div>
                    </div>
                    <a href="{{ route('nicu.admissions.index') }}" class="btn btn-sm btn-outline-warning">
                        <i class="bi bi-box-arrow-up-right"></i> NICU Module
                    </a>
                </div>

                @if ($nicuRecord->mother)
                    <div class="border-top pt-2 mt-2">
                        <small class="text-muted d-block mb-1"><strong><i class="bi bi-person-heart"></i> Mother</strong></small>
                        <div class="d-flex flex-wrap align-items-center gap-3">
                            <div>
                                <strong>{{ $nicuRecord->mother->patient_name }}</strong>
                                <span class="text-muted small ms-2">MRN: {{ $nicuRecord->mother->mrn }}</span>
                                @if ($nicuRecord->mother->mobileno)
                                    <span class="text-muted small ms-2"><i class="bi bi-telephone"></i> {{ $nicuRecord->mother->mobileno }}</span>
                                @endif
                            </div>
                            <a href="{{ route('patients.show', $nicuRecord->mother->id) }}" class="btn btn-sm btn-outline-primary">
                                <i class="bi bi-arrow-right"></i> Mother Profile
                            </a>
                            @if ($nicuRecord->motherIpd)
                                <a href="{{ route('ipd-patients.show', $nicuRecord->motherIpd->id) }}" class="btn btn-sm btn-outline-info">
                                    <i class="bi bi-clipboard2-pulse"></i> Mother's IPD ({{ $nicuRecord->motherIpd->ipd_no }})
                                </a>
                            @endif
                        </div>
                    </div>
                ...
                @if ($nicuRecord->resources->where('status', 'active')->count())
                    <div class="border-top pt-2 mt-2">
                        <small class="text-muted">Active resources:</small>
                        @foreach ($nicuRecord->resources->where('status', 'active') as $r)
                            <span class="badge bg-info">{{ $r->resource_type }}</span>
                        @endforeach
                    </div>
                @endif
            </div>
    ...
    @php
        // If this IPD belongs to a mother, show babies admitted under her
        $babies = \App\Models\Nicu\NicuAdmission::with('patient', 'resources')
            ->where('mother_ipd_patient_id', $iPDPatient->id)
            ->orWhere(function ($q) use ($iPDPatient) {
                $q->where('mother_patient_id', $iPDPatient->patient_id);
            })
            ->latest('id')->get();
    @endphp
    ...
    @if ($babies->count())
        <div class="card border-info bg-info bg-opacity-10 mb-3">
            <div class="card-body p-3">
                <h6 class="mb-2"><i class="bi bi-emoji-smile text-info"></i> Newborn(s) admitted to NICU under this mother
                    <span class="badge bg-info ms-1">{{ $babies->count() }}</span>
                </h6>
                <div class="table-responsive">
                    <table class="table table-sm mb-0 align-middle">
                        <thead class="table-light">
                            <tr><th>Baby ID</th><th>Name</th><th>Birth wt</th><th>GA</th><th>Status</th><th>Resources</th><th>Open</th></tr>
                        </thead>
                        <tbody>
                            @foreach ($babies as $b)
                                <tr>
                                    <td><strong>{{ $b->baby_id }}</strong></td>
                                    <td>{{ $b->patient->patient_name ?? '—' }}</td>
                                    <td>{{ $b->birth_weight_g ? number_format($b->birth_weight_g, 0) . 'g' : '—' }}</td>
                                    <td>{{ $b->gestational_age_weeks ? $b->gestational_age_weeks . 'w' : '—' }}</td>
                                    <td>
                                        @if ($b->is_critical)<span class="badge bg-danger">Critical</span>@endif
                                        @if ($b->is_preterm)<span class="badge bg-warning text-dark">Preterm</span>@endif
                                        @if ($b->is_low_birth_weight)<span class="badge bg-warning text-dark">LBW</span>@endif
                                    </td>
                                    <td>
                                        @foreach ($b->resources->where('status', 'active') as $r)
                                            <span class="badge bg-info bg-opacity-15 text-info">{{ $r->resource_type }}</span>
                                        @endforeach
                                    </td>
                                    <td>
                                        @if ($b->ipd_patient_id)
                                            <a href="{{ route('ipd-patients.show', $b->ipd_patient_id) }}" class="btn btn-sm btn-outline-info">
