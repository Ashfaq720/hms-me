<div class="table-responsive">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h6 class="mb-0">Round Doctor Entries</h6>
        </div>
        <div>
            @if (strcasecmp($iPDPatient->status, 'Admitted') === 0)
                <a data-size="lg" class="btn btn-primary px-2 w-100 w-sm-auto"
                    data-url="{{ route('ipd-patients.round-drs', $iPDPatient->id) }}" data-ajax-popup="true"
                    data-title="Add Round Dr Entry" data-bs-toggle="tooltip" title="Add Round Dr Entry">
                    <i class="bi bi-plus-lg me-1"></i> Add Round Dr
                </a>
            @endif
        </div>
    </div>
    ...
    <table class="table table-sm table-bordered align-middle">
        <thead class="table-light">
            <tr>
                ...
                <th width="8%">Visit #</th>
                <th width="25%">Clinical Observation</th>
                <th width="24%">Notes</th>
            </tr>
        </thead>
        <tbody>
            @forelse ($iPDPatient->roundDrs->sortByDesc('datetime') as $entry)
                <tr>
                    ...
                    <td>{{ $entry->clinical_observation ?? '-' }}</td>
                    <td>{{ $entry->notes ?? '-' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" class="text-center text-muted">No round doctor entries found.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
