...
<style>
    .bc-picker .bc-chip {
        ...
        padding: .35rem .7rem; border: 1px solid #dde2e7; border-radius: 99px;
        font-size: .82rem; cursor: pointer; background: #fff; user-select: none;
        transition: background .12s ease, border-color .12s ease, color .12s ease;
    }
    .bc-picker .bc-chip input { display: none; }
    .bc-picker .bc-chip:hover { border-color: #adb5bd; }
    .bc-picker .bc-chip.active {
        background: #dc3545; border-color: #dc3545; color: #fff; font-weight: 500;
    }
    .bc-picker .bc-chip.active::before {
        content: "\2713"; font-weight: 700;
    }
</style>
<script>
    // Toggle .active class on chip when its hidden checkbox flips
    document.addEventListener('change', e => {
        if (e.target && e.target.matches('.bc-picker input[type="checkbox"]')) {
...
<div class="row g-3 mb-3">
    ...
    <div class="col-md-4">
        ...
        {{-- Hidden 0-marker so an empty selection still posts an empty array (clears saved values) --}}
        <input type="hidden" name="blood_components[]" value="">
        <div class="d-flex flex-wrap gap-2 bc-picker">
            @foreach(($bloodComponents ?? \App\Models\Ot\OtSurgeryRequest::BLOOD_COMPONENTS) as $c)
                @php($isSel = in_array($c, $selectedComponents))
                <label class="bc-chip {{ $isSel ? 'active' : '' }}">
                    <input type="checkbox" name="blood_components[]" value="{{ $c }}" @checked($isSel)>
                    <span>{{ $c }}</span>
                </label>
            @endforeach
        </div>
        <small class="form-text text-muted">Click chips to toggle. Selected components will be reserved.</small>
    </div>
    <div class="col-md-2 d-flex align-items-end">
        <div class="form-check">
    ...
    <table class="table table-sm" id="equipment-table">
        ...
        <tbody id="equipment-rows">
            ...
            @forelse($eqRows as $i => $row)
                <tr class="eq-row">
                    <td>
                        <select name="equipments[{{ $i }}][ot_equipment_id]" class="form-select form-select-sm eq-select" data-name-target="equipments[{{ $i }}][equipment_name]">
                            <option value="">— Select equipment from inventory —</option>
                            @foreach($equipmentMaster ?? [] as $eq)
                                <option value="{{ $eq->id }}"
                                        data-name="{{ $eq->name }}"
                                        data-status="{{ $eq->status }}"
                                        data-room="{{ optional($eq->room)->name }}"
                                        @selected((string)($row['ot_equipment_id'] ?? '') === (string)$eq->id)>
                                    {{ $eq->name }}{{ $eq->code ? ' ['.$eq->code.']' : '' }}{{ $eq->status && $eq->status !== 'available' ? ' — '.ucfirst($eq->status) : '' }}
                                </option>
                            @endforeach
                        </select>
                        <input type="hidden" name="equipments[{{ $i }}][equipment_name]" value="{{ $row['equipment_name'] ?? '' }}">
                    </td>
...
(function () {
    ...
    // Build equipment <option> list once from the server-rendered master
    const equipmentOptions = {!! json_encode(($equipmentMaster ?? collect())->map(function ($e) { return ['id' => $e->id, 'name' => $e->name, 'code' => $e->code, 'status' => $e->status]; })->values()) !!};

    function buildEquipmentSelectOptions(selectedId = '') {
        let html = '<option value="">— Select equipment from inventory —</option>';
        for (const e of equipmentOptions) {
            const label = e.name
                + (e.code ? ' [' + e.code + ']' : '')
                + (e.status && e.status !== 'available' ? ' — ' + e.status.charAt(0).toUpperCase() + e.status.slice(1) : '');
            const sel = String(selectedId) === String(e.id) ? ' selected' : '';
            html += `<option value="${e.id}" data-name="${e.name}"${sel}>${label}</option>`;
        }
        return html;
    }

    // Mirror the selected option's name into the hidden equipment_name input
    function syncEquipmentNameFromSelect(sel) {
        const row = sel.closest('tr');
        if (!row) return;
        const opt = sel.options[sel.selectedIndex];
        const hidden = row.querySelector('input[type="hidden"][name$="[equipment_name]"]');
        if (hidden) hidden.value = opt ? (opt.dataset.name || '') : '';
    }
    document.addEventListener('change', e => {
        if (e.target.classList && e.target.classList.contains('eq-select')) {
            syncEquipmentNameFromSelect(e.target);
        }
    });
    // Initial sync for pre-populated rows
    document.querySelectorAll('select.eq-select').forEach(syncEquipmentNameFromSelect);
    ...
    if (addBtn && rows) {
        addBtn.addEventListener('click', () => {
            ...
            tr.innerHTML = `
                <td>
                    <select name="equipments[${idx}][ot_equipment_id]" class="form-select form-select-sm eq-select">
                        ${buildEquipmentSelectOptions()}
                    </select>
                    <input type="hidden" name="equipments[${idx}][equipment_name]" value="">
                </td>
