    ...
    <div class="container">
        <div class="app-page-head d-flex flex-wrap gap-3 align-items-center justify-content-between">
            ...
            @can('inventory.item.manage')
                @php
                    // Context-aware Add button — each type-view opens the right
                    // form so flags + linked rows are correct from the start.
                    $type = request('type');
                @endphp
                @switch($type)
                    @case('medicine')
                        {{-- Pharmacy form atomically creates medicine + inventory_item --}}
                        <a href="javascript:void(0);" class="btn btn-primary"
                           data-url="{{ route('admin.medicines.create') }}"
                           data-title="Create Medicine"
                           data-size="xl"
                           data-ajax-popup="true">
                            <i class="fi fi-rr-plus me-1"></i> Add Medicine
                        </a>
                        @break

                    @case('consumable')
                        <a href="{{ route('inventory.items.create', ['preset' => 'consumable']) }}"
                           class="btn btn-primary">
                            <i class="fi fi-rr-plus me-1"></i> Add Consumable
                        </a>
                        @break

                    @case('asset')
                        <a href="{{ route('inventory.items.create', ['preset' => 'asset']) }}"
                           class="btn btn-primary">
                            <i class="fi fi-rr-plus me-1"></i> Add Equipment / Asset
                        </a>
                        @break
                    ...
                    <table class="table align-middle mb-0">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>Category</th>
                                <th>Generic / Brand</th>
                                <th>Pharma Group / Composition</th>
                                <th class="text-end">Live Stock</th>
                                <th>UOM</th>
                        ...
                        <tbody>
                            @forelse ($items as $idx => $item)
                                <tr>
                                    ...
                                    <td>
                                        @if ($item->medicine)
                                            <small class="d-block">
                                                ...
                                                {{ $item->medicine->medicalGroup->name ?? '—' }}
                                            </small>
                                            <small class="text-muted">{{ $item->medicine->medicine_composition ?? '—' }}</small>
                                        @else
                                    ...
                                    <td class="text-end">
                                        @php
                                            $stock = (float) ($item->stock_total ?? 0);
                                            $reorder = (float) ($item->reorder_level ?? 0);
                                        @endphp
                                        @if ($stock <= 0)
                                            <span class="badge bg-danger" title="Across all warehouses">{{ number_format($stock, 2) }}</span>
                                        @elseif ($reorder > 0 && $stock <= $reorder)
                                            <span class="badge bg-warning" title="At or below reorder level">{{ number_format($stock, 2) }}</span>
                                        @else
                                            <span class="fw-semibold">{{ number_format($stock, 2) }}</span>
                                        @endif
                                    </td>
                                    <td>{{ $item->uom }}</td>
                            ...
                            @empty
                                <tr><td colspan="12" class="text-center py-4 text-muted">No items match your filter.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
