...
@section('title', 'Add Treatment History - Ipd')
    ...
    <div class="container-fluid py-3">
        ...
        <div class="card shadow-sm border-0 mb-3">
            <div class="card-body py-3 d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="mb-1 fw-bold">Add Treatment History</h4>
                    <div class="text-muted small">
                        {{ $ipdPatient->patient->patient_name ?? 'N/A' }} —
                        Ipd <strong>{{ $ipdPatient->ipd_no ?? '-' }}</strong>
                    </div>
                </div>
                <a href="{{ route('ipd-patients.show', $ipdPatient->id) }}"
        ...
        <form action="{{ route('ipd-patients.treatment-histories.store', $ipdPatient->id) }}" method="POST">
            @csrf
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    @include('ipd_patients.treatment-histories._fields')
                </div>
                <div class="card-footer bg-white border-0 d-flex justify-content-end gap-2 pb-3">
                    <a href="{{ route('ipd-patients.show', $ipdPatient->id) }}" class="btn btn-light">Cancel</a>
