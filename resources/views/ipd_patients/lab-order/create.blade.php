...
<div class="container-fluid py-3" style="padding-bottom:80px !important;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h4 class="mb-1"><i class="bi bi-clipboard2-pulse text-primary"></i> New Lab Order
                <small class="text-muted">for {{ optional($ipd->patient)->patient_name }}</small>
            </h4>
            <small class="text-muted">Pick investigations from any of the 9 lab types. Each line auto-creates a patient charge.</small>
        </div>
        <a href="{{ route('ipd-patients.show', $ipd->id) }}?tab=lab" class="btn btn-sm btn-outline-secondary">
    ...
    <form method="POST" action="{{ route('ipd-patients.lab-orders.store', $ipd->id) }}" id="labOrderForm">
        @csrf
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="doctor_id" class="form-select form-select-sm">
                            ...
                            @foreach ($doctors as $d)
                                <option value="{{ $d->id }}" @selected($ipd->doctor_id == $d->id)>{{ $d->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-2">
...
<div class="position-fixed bottom-0 start-0 end-0 bg-white shadow-lg border-top py-2 px-4 d-flex justify-content-between align-items-center" style="z-index:1030;">
    ...
    <div class="d-flex gap-2">
        <a href="{{ route('ipd-patients.show', $ipd->id) }}?tab=lab" class="btn btn-light btn-sm">Cancel</a>
