<form action="{{ route('ipd-patients.case-operations.store', $id) }}" method="POST">
    @csrf
    ...
    <div class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <select name="operation_type_id" id="operation_type_id"
                ...
                @foreach ($operationTypes as $type)
                    <option value="{{ $type->id }}" @selected(old('operation_type_id') == $type->id)>{{ $type->name }}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            ...
            <select name="operation_id" id="operation_id"
                ...
                @foreach ($operations as $op)
                    <option value="{{ $op->id }}" @selected(old('operation_id') == $op->id)>{{ $op->name }}</option>
                @endforeach
            </select>
            @error('operation_id')
                <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="col-md-6">
            ...
            <select name="operation_procedure_id" id="operation_procedure_id"
                ...
                @foreach ($procedures as $proc)
                    <option value="{{ $proc->id }}" @selected(old('operation_procedure_id') == $proc->id)>{{ $proc->name }}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            ...
            <select name="operation_theatre_id" id="operation_theatre_id"
                ...
                @foreach ($theatres as $theatre)
                    <option value="{{ $theatre->id }}" @selected(old('operation_theatre_id') == $theatre->id)>{{ $theatre->name }}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            ...
            <select name="assign_doctor_id" id="assign_doctor_id"
                ...
                @foreach ($doctors as $doctor)
                    <option value="{{ $doctor->id }}" @selected(old('assign_doctor_id') == $doctor->id)>{{ $doctor->name }}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            ...
            <select name="assistant_doctor_id" id="assistant_doctor_id"
                ...
                <option value="">-- Select Doctor --</option>
                @foreach ($doctors as $doctor)
                    <option value="{{ $doctor->id }}" @selected(old('assistant_doctor_id') == $doctor->id)>{{ $doctor->name }}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            ...
            <select name="main_surgeon_id" id="main_surgeon_id"
                ...
                @foreach ($doctors as $doctor)
                    <option value="{{ $doctor->id }}" @selected(old('main_surgeon_id') == $doctor->id)>{{ $doctor->name }}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            ...
            <select name="anesthesiologist_id" id="anesthesiologist_id"
                ...
                @foreach ($doctors as $doctor)
                    <option value="{{ $doctor->id }}" @selected(old('anesthesiologist_id') == $doctor->id)>{{ $doctor->name }}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="ot_technician" id="ot_technician"
                ...
                value="{{ old('ot_technician') }}" placeholder="Enter OT Technician name">
            @error('ot_technician')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-6">
            ...
            <select name="status" id="status"
                ...
                @foreach (['Scheduled', 'In Progress', 'Completed', 'Cancelled'] as $s)
                    <option value="{{ $s }}" @selected(old('status', 'Scheduled') == $s)>{{ $s }}</option>
                @endforeach
            </select>
            @error('status')
                <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="col-12">
            ...
            <textarea name="remarks" id="remarks" class="form-control @error('remarks') is-invalid @enderror"
                rows="2" placeholder="Enter remarks">{{ old('remarks') }}</textarea>
            @error('remarks')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
