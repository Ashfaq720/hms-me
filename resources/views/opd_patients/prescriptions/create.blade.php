...
@section('content')
    <div class="rx-page">
        <div class="container">
            ...
            <form action="{{ route('opd-patients.prescriptions.store', $opdPatient->id) }}" method="POST"
                id="prescriptionForm">
                @csrf
                ...
                <div class="rx-header">
                    <div class="rx-header-left">
                        ...
                        <div>
                            ...
                            @if ($opdPatient->doctor && $opdPatient->doctor->department)
                                <p class="rx-hospital-sub">{{ $opdPatient->doctor->department->name ?? '' }}</p>
                            @endif
                    ...
                    <div class="rx-doctor-info">
                        ...
                        <div id="doctorDisplay">
                            @if ($opdPatient->doctor)
                                <p class="rx-doctor-name">{{ $opdPatient->doctor->name }}</p>
                                <p class="rx-doctor-desg">{{ $opdPatient->doctor?->designation?->name ?? '' }}</p>
                                <p class="rx-doctor-desg">Reg. No: {{ $opdPatient->doctor?->registration_no ?? '-' }}</p>
                            @else
                ...
                <div class="row mt-0">
                    ...
                    <div class="col-lg-7">
                        ...
                        <div class="rx-notes-card">
                            ...
                            <div class="rx-notes-title"><i class="bi bi-journal-medical"></i> Diagnosis &amp; Clinical Notes</div>
                            ...
                            <div class="row g-2 mb-2">
                                <div class="col-md-4">
                                    <label class="form-label fw-semibold small mb-1">ICD-10 Code</label>
                                    <input type="text" name="icd10_code" id="icd10_code"
                                        class="form-control form-control-sm"
                                        value="{{ old('icd10_code') }}" placeholder="e.g. J06.9" maxlength="20">
                                </div>
                                <div class="col-md-8">
                                    <label class="form-label fw-semibold small mb-1">ICD-10 Description</label>
                                    <input type="text" name="icd10_description" id="icd10_description"
                                        class="form-control form-control-sm"
                                        value="{{ old('icd10_description') }}" placeholder="Diagnosis description">
                                </div>
                            </div>
                            ...
                            <div class="rx-notes-title"><i class="bi bi-radioactive"></i> Radiology Orders</div>
                            <textarea name="radiology_orders" id="radiology_orders"
                                class="form-control border-0 bg-transparent p-0" rows="2"
                                placeholder="e.g. Chest X-ray, MRI Brain…" style="font-size:14px; resize:none;">{{ old('radiology_orders') }}</textarea>
                        </div>
                        ...
                        <div class="rx-notes-card">
                            ...
                            <textarea name="advice" id="advice" class="form-control border-0 bg-transparent p-0" rows="2"
                                placeholder="Lifestyle, diet, rest, instructions…" style="font-size:14px; resize:none;">{{ old('advice') }}</textarea>
                            <hr class="my-2">
                            <div class="d-flex align-items-center gap-2 flex-wrap">
                                <label for="next_visit" class="form-label mb-0 fw-semibold" style="white-space:nowrap;">Next Visit:</label>
                                <input type="date" name="next_visit" id="next_visit"
                                    class="form-control form-control-sm border-0 bg-transparent"
                                    value="{{ old('next_visit') }}" style="max-width: 180px;">
                            </div>
                            <hr class="my-2">
                            <div class="rx-notes-title small"><i class="bi bi-calendar2-check"></i> Follow-up Note</div>
                            <textarea name="follow_up_note" id="follow_up_note"
                                class="form-control border-0 bg-transparent p-0" rows="2"
                                placeholder="Instructions for the next visit…" style="font-size:14px; resize:none;">{{ old('follow_up_note') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                {{-- ══════════ FOOTER BUTTONS ══════════ --}}
                <div class="rx-footer">
                    {{-- <a href="{{ route('opd-patients.show', $opdPatient->id) }}" class="btn-rx-outline">
                        View Details
                    </a> --}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('addMedicineBtn').addEventListener('click', function() {
                ...
                if (dosage !== '0 + 0 + 0 + 0') display += ` --- [ ${dm} - ${da} - ${de} - ${dn} ]`;
