...
<form action="{{ route('front_desk.registration.store') }}" method="POST" enctype="multipart/form-data" id="fdRegForm"
        ...
        <div class="row g-2">
            ...
            <div class="col-12 col-md-6">
                ...
                <select name="registration_type" id="registration_type"
                    ...
                    <option value="NEW_PATIENT"
                        {{ old('registration_type', 'NEW_PATIENT') == 'NEW_PATIENT' ? 'selected' : '' }}>New
                        Patient</option>
                    <option value="EXISTING_PATIENT"
                        {{ old('registration_type') == 'EXISTING_PATIENT' ? 'selected' : '' }}>Existing
                        Patient</option>
                    <option value="UNKNOWN"
                        {{ old('registration_type') == 'UNKNOWN' ? 'selected' : '' }}>Unknown /
                        Emergency</option>
                </select>
            ...
            <div class="col-12 col-md-6">
                ...
                <select name="patient_type" id="patient_type"
                    ...
                    <option value="OPD" {{ old('patient_type') == 'OPD' ? 'selected' : '' }}>OPD – Out Patient
                    </option>
                    <option value="Ipd" {{ old('patient_type') == 'Ipd' ? 'selected' : '' }}>IPD – In Patient
                    </option>
            ...
            {{-- Existing patient picker (EXISTING_PATIENT only) --}}
            <div class="col-12" id="existingPatientWrap" style="display:none;">
                <label for="patient_id" class="form-label form-label-sm fw-medium mb-1">
                    Search Patient
                    <span class="text-muted fw-normal small ms-1">— by name, MRN, mobile, Health Card No, or NID</span>
                ...
                <div id="existingPatientCard" class="fd-card mt-2 d-flex flex-wrap gap-3 small" style="display:none!important;">
                    <div><span class="text-muted">Health Card:</span> <strong id="epHealthCard">—</strong></div>
                    <div><span class="text-muted">Gender:</span> <strong id="epGender">—</strong></div>
                    <div><span class="text-muted">Age:</span> <strong id="epAge">—</strong></div>
                    <div><span class="text-muted">Last Visit:</span> <strong id="epLastVisit">—</strong></div>
                    <div><span class="text-muted">Prev. Dept:</span> <strong id="epLastDept">—</strong></div>
                </div>
            </div>
            ...
            <div class="col-12 col-md-6">
                ...
                <div class="input-group input-group-sm">
                    ...
                    <input type="text" name="contact_no" id="patient_contact"
                        ...
                        placeholder="01XXXXXXXXX" required maxlength="20" inputmode="tel" autocomplete="off">
            ...
            {{-- Address / Guardian / Emergency — new patient only --}}
            ...
            <div class="col-12 col-md-4">
                ...
                <select name="discount_type" id="discount_type"
                    ...
                    <option value="">-- Select --</option>
                    <option value="SELF" {{ old('discount_type') == 'SELF' ? 'selected' : '' }}>Self Pay
                    </option>
    ...
    <div class="fd-sec">
        ...
        <div id="ipdFields" class="mt-2" hidden>
            <div class="fd-card">
                <div class="fd-sec-label mb-2"><span class="fd-sec-num" style="background:#198754;">↳</span> Bed
                    Assignment</div>
                <div class="row g-2">
                    <div class="col-12 col-md-6">
                        <label class="form-label form-label-sm fw-medium mb-1">Bed <span
                    ...
                    <div class="col-6 col-md-3">
                        <label class="form-label form-label-sm fw-medium mb-1">From <span
                                class="text-danger">*</span></label>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="bed_remarks" class="form-control form-control-sm" rows="2" placeholder="Reason / notes">{{ old('bed_remarks') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    (function() {
        ...
        function handleRegType() {
            ...
            // Init Select2 on #patient_id the first time the wrapper becomes visible
            // (deferred because Select2 measures width; must be visible at init time)
            if (isExisting && !$('#patient_id').hasClass('select2-hidden-accessible')) {
                $('#patient_id').select2({
                    theme: 'bootstrap-5',
                    width: '100%',
            ...
            // Address / guardian / emergency only for new / unknown
            if (newPatientExtraFields) {
                isExisting ? hide(newPatientExtraFields) : show(newPatientExtraFields);
            }
            ...
            if (isExisting) {
                ...
                document.getElementById('existingPatientCard').style.removeProperty('display');
            } else {
                document.getElementById('existingPatientCard').style.setProperty('display', 'none', 'important');
            }
        }
        ...
        function toggleByType() {
            ...
            if (ipdFields) {
                ipdFields.hidden = !isIpd;
            }
            if (erFields) {
                erFields.hidden = !isEr;
            }
            toggleShiftSlot();
            if (billingSection) {
                billingSection.style.display = isOpd ? '' : 'none';
            }

            var visitTypeWrap   = document.getElementById('visitTypeWrap');
            var opdPriorityWrap = document.getElementById('opdPriorityWrap');
            if (visitTypeWrap)   visitTypeWrap.hidden   = !isOpd;
            if (opdPriorityWrap) opdPriorityWrap.hidden = !isOpd;
            if (!isOpd) {
        ...
        /* ── Shift / Slot show-hide based on Source (booking_status) ─────────── */
        function toggleShiftSlot() {
            var isAppointment = document.getElementById('booking_status').value === 'PRE_BOOK';

            if (shiftField)   shiftField.hidden   = !isAppointment;
            if (opdSlotField) opdSlotField.hidden  = !isAppointment;
        ...
        $('#patient_id').on('change', function() {
            ...
            $.get("{{ route('front_desk.patients.search') }}", { id: id }, function(p) {
                ...
                if (document.getElementById('patient_name')) $('#patient_name').val(p.patient_name);
                if (document.getElementById('patient_contact')) $('#patient_contact').val(p.mobileno);
                if (document.getElementById('gender')) $('#gender').val(p.gender);
                if (document.getElementById('blood_group')) $('#blood_group').val(p.blood_group);
                if (dobEl) { dobEl.value = p.dob || ''; if (ageDisplay) ageDisplay.textContent = p.dob ? 'Age: ' + calcAge(p.dob) : ''; }
                if (document.getElementById('discount_type')) { $('#discount_type').val(p.discount_type).trigger('change'); }
                if (document.getElementById('organization_name')) $('#organization_name').val(p.organization_name || '');
                if (document.getElementById('organization_id')) $('#organization_id').val(p.organization_id || '');
                if (document.getElementById('organization_api_link')) $('#organization_api_link').val(p.organization_api_link || '');

                // Show summary card
                var age = p.dob ? calcAge(p.dob) : '—';
                document.getElementById('epHealthCard').textContent = p.health_card_no || '—';
                document.getElementById('epGender').textContent = p.gender || '—';
                document.getElementById('epAge').textContent = age || '—';
        ...
        function checkActiveToken() {
            ...
            if (!patientId || !deptId || !dateVal || regTypeEl.value !== 'EXISTING_PATIENT' || patientTypeEl.value !== 'OPD') {
