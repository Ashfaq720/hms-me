...
(function () {
    ...
    /* ─── Select2 helpers (lazy — init only when element is visible) ─── */
    ...
    function initPatientSelect2() {
        ...
        if (!$('#patient_id').hasClass('select2-hidden-accessible')) {
            $('#patient_id').select2({ theme: 'bootstrap-5', width: '100%', placeholder: 'Search by name, MRN or mobile', allowClear: true, dropdownParent: getModalParent() });
        }
    }
    function initBedSelect2() {
        if (typeof $ === 'undefined' || !$.fn.select2) return;
        if (!$('#bed_id').hasClass('select2-hidden-accessible')) {
            $('#bed_id').select2({ theme: 'bootstrap-5', width: '100%', placeholder: '-- Select Available Bed --', allowClear: true, dropdownParent: getModalParent() });
        }
    }
