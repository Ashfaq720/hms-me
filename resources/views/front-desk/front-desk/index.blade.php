...
@section('content')
                                ...
                                <tbody>
                                    @forelse ($opdPatients as $i => $opd)
                                        @php
                                            ...
                                            $opdSlot = $opd->slot_time_from
                                                ? \Carbon\Carbon::parse($opd->slot_time_from)->format('h:i A') . ($opd->slot_time_to ? ' – ' . \Carbon\Carbon::parse($opd->slot_time_to)->format('h:i A') : '')
                                                : '—';
                                        @endphp
                                        <tr>
                                            ...
                                            <td class="text-end pe-3">
                                                <div class="d-inline-flex gap-1">
                                                    ...
                                                    <button type="button"
                                                        class="btn btn-sm btn-outline-primary opd-view-btn"
                                                        title="View"
                                                        data-token="{{ $opd->token_no ?? '—' }}"
                                                        data-patient="{{ $opd->patient->patient_name ?? '—' }}"
                                                        data-mrn="{{ $opd->patient->mrn ?? '—' }}"
                                                        data-contact="{{ $opd->patient->mobileno ?? '—' }}"
                                                        data-doctor="{{ $opd->doctor->name ?? '—' }}"
                                                        data-department="{{ $opd->department->name ?? '—' }}"
                                                        data-date="{{ $opd->date ? \Carbon\Carbon::parse($opd->date)->format('d M Y') : '—' }}"
                                                        data-visit-type="{{ $opd->visit_type_label }}"
                                                        data-slot="{{ $opdSlot }}"
                                                        data-priority="{{ $opd->priority ?? '—' }}"
                                                        data-status="{{ $opd->status ?? 'Registered' }}"
                                                        data-status-color="{{ $sc }}"
                                                        data-remarks="{{ $opd->remarks ?? '' }}"
                                                        data-complaint="{{ $opd->chief_complaint ?? '' }}"
                                                        data-referral="{{ $opd->referral_source ?? '' }}">
                                                        <i class="bi bi-eye"></i>
                                                    </button>
                                                </div>
                                            </td>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('ipd-patients.show', $ipd->id) }}"
                                                    class="fw-semibold text-success font-monospace"
                                                    style="text-decoration:none;">
                                                    {{ $ipd->ipd_no ?? '—' }}
                                                </a>
                                            </td>
                                            ...
                                            <td class="text-end pe-3">
                                                <div class="d-inline-flex gap-1">
                                                    ...
                                                    <button type="button"
                                                        class="btn btn-sm btn-outline-success ipd-view-btn"
                                                        title="View"
                                                        data-ipd-no="{{ $ipd->ipd_no ?? '—' }}"
                                                        data-patient="{{ $ipd->patient->patient_name ?? '—' }}"
                                                        data-mrn="{{ $ipd->patient->mrn ?? '—' }}"
                                                        data-contact="{{ $ipd->patient->mobileno ?? '—' }}"
                                                        data-doctor="{{ $ipd->doctor->name ?? '—' }}"
                                                        data-department="{{ $ipd->department->name ?? '—' }}"
                                                        data-admission="{{ $ipd->admission_date ? \Carbon\Carbon::parse($ipd->admission_date)->format('d M Y') : '—' }}"
                                                        data-discharge="{{ $ipd->possible_discharge_date ? \Carbon\Carbon::parse($ipd->possible_discharge_date)->format('d M Y') : '—' }}"
                                                        data-admission-type="{{ $ipd->admission_type ?? '—' }}"
                                                        data-status="{{ $ipd->status ?? 'Admitted' }}"
                                                        data-status-color="{{ $sc }}"
                                                        data-history="{{ $ipd->patient_history ?? '' }}"
                                                        data-remarks="{{ $ipd->remarks ?? '' }}">
                                                        <i class="bi bi-eye"></i>
                                                    </button>
    ...
    {{-- IPD Patient Detail Modal --}}
    <div class="modal fade" id="ipdDetailModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width:560px;">
            <div class="modal-content border-0 shadow">
                <div class="modal-header border-0 pb-0 pt-4 px-4" style="background:#f0fdf4;border-radius:12px 12px 0 0;">
                    <div>
                        <h5 class="modal-title fw-bold mb-0 text-success">IPD Admission Details</h5>
                        <div class="text-muted small mt-1 font-monospace" id="ipd-no-line"></div>
                    </div>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body px-4 pt-3 pb-2">
                    <div class="d-flex align-items-center gap-3 mb-4 p-3 rounded-3" style="background:#f0fdf4;">
                        <div class="detail-avatar detail-avatar-green" id="ipd-avatar"></div>
                        <div>
                            <div class="fw-bold fs-6" id="ipd-patient"></div>
                            <div class="text-muted small font-monospace" id="ipd-mrn"></div>
                            <div class="text-muted small" id="ipd-contact"></div>
                        </div>
                        <div class="ms-auto text-end">
                            <span class="badge rounded-pill" id="ipd-status-badge"></span>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-person-badge me-1"></i>Doctor</div>
                                <div class="detail-value" id="ipd-doctor"></div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-building me-1"></i>Department</div>
                                <div class="detail-value" id="ipd-department"></div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-calendar-check me-1"></i>Admission Date</div>
                                <div class="detail-value" id="ipd-admission"></div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-calendar-x me-1"></i>Expected Discharge</div>
                                <div class="detail-value" id="ipd-discharge"></div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-hospital me-1"></i>Admission Type</div>
                                <div class="detail-value" id="ipd-admission-type"></div>
                            </div>
                        </div>
                    </div>

                    <div id="ipd-history-wrap" class="detail-field mb-3" style="display:none;">
                        <div class="detail-label"><i class="bi bi-file-medical me-1"></i>Patient History</div>
                        <div class="detail-value" id="ipd-history" style="white-space:pre-line;"></div>
                    </div>

                    <div id="ipd-remarks-wrap" class="detail-field mb-3" style="display:none;">
                        <div class="detail-label"><i class="bi bi-chat-text me-1"></i>Remarks</div>
                        <div class="detail-value" id="ipd-remarks" style="white-space:pre-line;"></div>
                    </div>
                </div>
                <div class="modal-footer border-0 pt-0 px-4 pb-4">
                    <button type="button" class="btn btn-light btn-sm px-4" data-bs-dismiss="modal">Close</button>
    ...
    {{-- OPD Patient Detail Modal --}}
    <div class="modal fade" id="opdDetailModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width:560px;">
            <div class="modal-content border-0 shadow">
                <div class="modal-header border-0 pb-0 pt-4 px-4" style="background:#eff6ff;border-radius:12px 12px 0 0;">
                    <div>
                        <h5 class="modal-title fw-bold mb-0 text-primary">OPD Visit Details</h5>
                        <div class="text-muted small mt-1 font-monospace" id="opd-token-line"></div>
                    </div>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body px-4 pt-3 pb-2">
                    <div class="d-flex align-items-center gap-3 mb-4 p-3 rounded-3" style="background:#eff6ff;">
                        <div class="detail-avatar detail-avatar-blue" id="opd-avatar"></div>
                        <div>
                            <div class="fw-bold fs-6" id="opd-patient"></div>
                            <div class="text-muted small font-monospace" id="opd-mrn"></div>
                            <div class="text-muted small" id="opd-contact"></div>
                        </div>
                        <div class="ms-auto text-end">
                            <span class="badge rounded-pill" id="opd-status-badge"></span>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-person-badge me-1"></i>Doctor</div>
                                <div class="detail-value" id="opd-doctor"></div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-building me-1"></i>Department</div>
                                <div class="detail-value" id="opd-department"></div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-calendar3 me-1"></i>Visit Date</div>
                                <div class="detail-value" id="opd-date"></div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-clock me-1"></i>Time Slot</div>
                                <div class="detail-value font-monospace" id="opd-slot"></div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-arrow-repeat me-1"></i>Visit Type</div>
                                <div class="detail-value" id="opd-visit-type"></div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="detail-field">
                                <div class="detail-label"><i class="bi bi-flag me-1"></i>Priority</div>
                                <div class="detail-value" id="opd-priority"></div>
                            </div>
                        </div>
                    </div>

                    <div id="opd-complaint-wrap" class="detail-field mb-3" style="display:none;">
                        <div class="detail-label"><i class="bi bi-file-medical me-1"></i>Chief Complaint</div>
                        <div class="detail-value" id="opd-complaint" style="white-space:pre-line;"></div>
                    </div>

                    <div id="opd-referral-wrap" class="detail-field mb-3" style="display:none;">
                        <div class="detail-label"><i class="bi bi-send me-1"></i>Referral Source</div>
                        <div class="detail-value" id="opd-referral"></div>
                    </div>

                    <div id="opd-remarks-wrap" class="detail-field mb-3" style="display:none;">
                        <div class="detail-label"><i class="bi bi-chat-text me-1"></i>Remarks</div>
                        <div class="detail-value" id="opd-remarks" style="white-space:pre-line;"></div>
                    </div>
                </div>
                <div class="modal-footer border-0 pt-0 px-4 pb-4">
                    <button type="button" class="btn btn-light btn-sm px-4" data-bs-dismiss="modal">Close</button>
    ...
    <style>
        ...
        .detail-avatar-green { background: #dcfce7; color: #15803d; }
    ...
    @push('scripts')
        ...
        /* ── IPD Patient Detail Modal ── */
        document.querySelectorAll('.ipd-view-btn').forEach(function (btn) {
            btn.addEventListener('click', function () {
                var d = this.dataset;

                document.getElementById('ipd-avatar').textContent         = (d.patient || 'P').charAt(0).toUpperCase();
                document.getElementById('ipd-patient').textContent        = d.patient;
                document.getElementById('ipd-mrn').textContent            = d.mrn !== '—' ? 'MRN: ' + d.mrn : '';
                document.getElementById('ipd-contact').textContent        = d.contact;
                document.getElementById('ipd-doctor').textContent         = d.doctor;
                document.getElementById('ipd-department').textContent     = d.department;
                document.getElementById('ipd-admission').textContent      = d.admission;
                document.getElementById('ipd-discharge').textContent      = d.discharge;
                document.getElementById('ipd-admission-type').textContent = d.admissionType;
                document.getElementById('ipd-no-line').textContent        = 'IPD No: ' + d.ipdNo;

                var badge = document.getElementById('ipd-status-badge');
                badge.textContent = d.status;
                badge.className = 'badge rounded-pill text-bg-' + d.statusColor + '-subtle border border-' + d.statusColor + '-subtle text-' + d.statusColor;

                var hWrap = document.getElementById('ipd-history-wrap');
                if (d.history && d.history.trim()) {
                    document.getElementById('ipd-history').textContent = d.history;
                    hWrap.style.display = '';
                } else {
                    hWrap.style.display = 'none';
                }

                var rWrap = document.getElementById('ipd-remarks-wrap');
                if (d.remarks && d.remarks.trim()) {
                    document.getElementById('ipd-remarks').textContent = d.remarks;
                    rWrap.style.display = '';
                } else {
                    rWrap.style.display = 'none';
                }

                new bootstrap.Modal(document.getElementById('ipdDetailModal')).show();
            });
        });

        /* ── OPD Patient Detail Modal ── */
        document.querySelectorAll('.opd-view-btn').forEach(function (btn) {
            btn.addEventListener('click', function () {
                var d = this.dataset;

                document.getElementById('opd-avatar').textContent      = (d.patient || 'P').charAt(0).toUpperCase();
                document.getElementById('opd-patient').textContent     = d.patient;
                document.getElementById('opd-mrn').textContent         = d.mrn !== '—' ? 'MRN: ' + d.mrn : '';
                document.getElementById('opd-contact').textContent     = d.contact;
                document.getElementById('opd-doctor').textContent      = d.doctor;
                document.getElementById('opd-department').textContent  = d.department;
                document.getElementById('opd-date').textContent        = d.date;
                document.getElementById('opd-slot').textContent        = d.slot;
                document.getElementById('opd-visit-type').textContent  = d.visitType;
                document.getElementById('opd-priority').textContent    = d.priority;
                document.getElementById('opd-token-line').textContent  = 'Token: ' + d.token;

                var badge = document.getElementById('opd-status-badge');
                badge.textContent = d.status;
                badge.className = 'badge rounded-pill text-bg-' + d.statusColor + '-subtle border border-' + d.statusColor + '-subtle text-' + d.statusColor;

                var cWrap = document.getElementById('opd-complaint-wrap');
                if (d.complaint && d.complaint.trim()) {
                    document.getElementById('opd-complaint').textContent = d.complaint;
                    cWrap.style.display = '';
                } else {
                    cWrap.style.display = 'none';
                }

                var refWrap = document.getElementById('opd-referral-wrap');
                if (d.referral && d.referral.trim()) {
                    document.getElementById('opd-referral').textContent = d.referral;
                    refWrap.style.display = '';
                } else {
                    refWrap.style.display = 'none';
                }

                var rWrap = document.getElementById('opd-remarks-wrap');
                if (d.remarks && d.remarks.trim()) {
                    document.getElementById('opd-remarks').textContent = d.remarks;
                    rWrap.style.display = '';
                } else {
                    rWrap.style.display = 'none';
                }

                new bootstrap.Modal(document.getElementById('opdDetailModal')).show();
            });
        });
        </script>
    @endpush
