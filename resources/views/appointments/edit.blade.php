...
@section('content')
    <div class="container-fluid py-4 apt-form-page">
        <div class="page-head d-flex align-items-center justify-content-between mb-4">
            <div>
                <h3 class="mb-1 fw-bold text-dark">Edit Appointment</h3>
                ...
                <div class="form-card__body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select select2 @error('patient_id') is-invalid @enderror" name="patient_id" data-placeholder="--Select Patient--">
                                <option value="">---Select Patient---</option>
                                @foreach ($patients as $p)
                                    <option value="{{ $p->id }}" {{ old('patient_id', $appointment->patient_id) == $p->id ? 'selected' : '' }}>{{ $p->patient_name }}</option>
                                @endforeach
                            </select>
                            @error('patient_id')
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select select2 @error('doctor') is-invalid @enderror" name="doctor" data-placeholder="--Select Doctor--">
                                <option value="">---Select Doctor---</option>
                                @foreach ($doctors as $d)
                                    <option value="{{ $d->id }}" {{ old('doctor', $appointment->doctor) == $d->id ? 'selected' : '' }}>{{ $d->name }}</option>
                                @endforeach
                            </select>
                            @error('doctor')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.01" class="form-control @error('amount') is-invalid @enderror" name="amount" value="{{ old('amount', $appointment->amount) }}" placeholder="0.00">
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        @php
                            $currentSlot = null;
                            if ($appointment->slot_time_from && $appointment->slot_time_to) {
                                $currentSlot = substr($appointment->slot_time_from, 0, 5) . '|' . substr($appointment->slot_time_to, 0, 5);
                            }
                        @endphp
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('specialist') is-invalid @enderror" name="specialist" value="{{ old('specialist', $appointment->specialist) }}" placeholder="e.g. Cardiology">
                            @error('specialist')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="form-card__body">
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="date" class="form-control @error('date') is-invalid @enderror" name="date" id="date_field" value="{{ old('date', $appointment->date?->format('Y-m-d')) }}">
                            @error('date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                         ...
                         <div class="col-md-4">
                            ...
                            <select class="form-select @error('shift_id') is-invalid @enderror" name="shift_id" id="shift_select" data-current="{{ old('shift_id', $appointment->shift_id) }}">
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select @error('slot') is-invalid @enderror" name="slot" id="slot_select" data-current="{{ old('slot', $currentSlot) }}">
                                <option value="">---Select Slot---</option>
                                @if ($currentSlot)
                                    @php
                                        [$cf, $ct] = explode('|', $currentSlot);
                                        $label = \Carbon\Carbon::parse($cf)->format('h:i A') . ' - ' . \Carbon\Carbon::parse($ct)->format('h:i A');
                                    @endphp
                                    <option value="{{ $currentSlot }}" selected>{{ $label }}</option>
                                @endif
                            </select>
                            @error('slot')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="time" class="form-control @error('time') is-invalid @enderror" name="time" id="time_field" value="{{ old('time', $appointment->time ? substr($appointment->time, 0, 5) : '') }}" readonly>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select @error('priority') is-invalid @enderror" name="priority">
                                ...
                                @foreach ($priorities as $pr)
                                    <option value="{{ $pr->name }}" {{ old('priority', $appointment->priority) == $pr->name ? 'selected' : '' }}>
                                        {{ $pr->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select @error('source') is-invalid @enderror" name="source">
                                ...
                                @foreach (['Walk-in', 'Phone', 'Web'] as $src)
                                    <option value="{{ $src }}" {{ old('source', $appointment->source) == $src ? 'selected' : '' }}>{{ $src }}</option>
                                @endforeach
                            </select>
                            @error('source')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('live_consult') is-invalid @enderror" name="live_consult">
                                ...
                                @foreach (['Zoom', 'Meet', 'None'] as $lc)
                                    <option value="{{ $lc }}" {{ old('live_consult', $appointment->live_consult) == $lc ? 'selected' : '' }}>{{ $lc }}</option>
                                @endforeach
                            </select>
                            @error('live_consult')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('appointment_status') is-invalid @enderror" name="appointment_status">
                                <option value="">---Select Status---</option>
                                @foreach (['Pending', 'Approved', 'Rejected', 'Cancelled'] as $status)
                                    <option value="{{ $status }}" {{ old('appointment_status', $appointment->appointment_status) == $status ? 'selected' : '' }}>{{ $status }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="row g-3">
                        <div class="col-md-3">
                            ...
                            <select class="form-select @error('visit_status') is-invalid @enderror" name="visit_status">
                                ...
                                @foreach ($visitStatuses as $val => $label)
                                    <option value="{{ $val }}" {{ old('visit_status', $appointment->visit_status) == $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('visit_status')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-select @error('is_opd') is-invalid @enderror" name="is_opd">
                                <option value="Yes" {{ old('is_opd', $appointment->is_opd) == 'Yes' ? 'selected' : '' }}>Yes</option>
                                <option value="No" {{ old('is_opd', $appointment->is_opd) == 'No' ? 'selected' : '' }}>No</option>
                            </select>
                            @error('is_opd')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-select @error('is_ipd') is-invalid @enderror" name="is_ipd">
                                <option value="Yes" {{ old('is_ipd', $appointment->is_ipd) == 'Yes' ? 'selected' : '' }}>Yes</option>
                                <option value="No" {{ old('is_ipd', $appointment->is_ipd) == 'No' ? 'selected' : '' }}>No</option>
                            </select>
                            @error('is_ipd')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-select @error('is_queue') is-invalid @enderror" name="is_queue">
                                <option value="0" {{ (string) old('is_queue', $appointment->is_queue ?? 0) === '0' ? 'selected' : '' }}>No</option>
                                <option value="1" {{ (string) old('is_queue', $appointment->is_queue ?? 0) === '1' ? 'selected' : '' }}>Yes</option>
                            </select>
                            @error('is_queue')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea class="form-control @error('message') is-invalid @enderror" name="message" rows="2" placeholder="Optional note or message">{{ old('message', $appointment->message) }}</textarea>
                            @error('message')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
@push('styles')
    <style>
        .apt-form-page .page-head h3 { color: #2b335d; }
        .apt-form-page .breadcrumb-item a { color: #6b7390; text-decoration: none; }
        .apt-form-page .breadcrumb-item.active { color: #2b335d; }
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const $doctor = $('select[name="doctor"]');
            ...
            function refreshSlots() {
                ...
                const current = $slot.data('current') || '';
                ...
                fetch(slotsUrl, {
                    ...
                    .then(list => {
                        ...
                        list.forEach(s => {
                            ...
                            if (val === current) { opt.selected = true; matched = true; }
                            $slot[0].appendChild(opt);
                        });
                        if (!matched) $slot.data('current', '');
                    });
            }

            $doctor.on('change', function () { $shift.data('current', ''); $slot.data('current', ''); refreshShifts().then(refreshSlots); });
            $shift.on('change', function () { $slot.data('current', ''); refreshSlots(); });
            $date.on('change', refreshSlots);

            // Auto-fill Time when a Slot is chosen
            $slot.on('change', function () {
                const v = $slot.val();
                $('#time_field').val(v && v.includes('|') ? v.split('|')[0] : '');
            });

            // Initial load: pick up saved doctor -> fetch shifts -> fetch slots
