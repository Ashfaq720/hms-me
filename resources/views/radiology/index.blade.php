...
@section('content')
    <div class="container-fluid">
        <div class="app-page-head d-flex flex-wrap gap-3 align-items-center justify-content-between">
            <div class="clearfix">
                <h1 class="app-page-title">Radiology Orders</h1>
            </div>
            <div>
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createModal">
                    <i class="bi bi-plus-lg me-1"></i> Create Radiology Order
                </button>
            </div>
        </div>
            ...
            <div class="col-12">
                <div class="card overflow-hidden">
                    <div
                        ...
                        <h6 class="card-title mb-2">Radiology Order List</h6>
                        <input type="text" id="radiologySearch" class="form-control form-control-sm mb-2"
                            style="max-width: 260px;" placeholder="Search...">
                    </div>

                    <div class="card-body px-3 pt-2 pb-0 gradient-layer">
                        <table id="radiologyTable" class="table table-striped display table-row-rounded">
                            ...
                            <tbody id="radiologyTbody">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @forelse($o->requests as $r)
                                                <span
                                                    class="badge bg-success m-1">{{ optional($r->labInvestigation)->name ?? '-' }}</span>
                                            @empty
                                                -
                                            @endforelse
                                        </td>
                                        <td class="text-nowrap">
                                            <a href="{{ route('radiology.show', $o->id) }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="11" class="text-center">No radiology orders found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                <div class="modal-content">
                    <form action="{{ route('radiology.store') }}" method="POST">
                        ...
                        <div class="modal-header d-flex align-items-center">
                            <h5 class="modal-title">Create Radiology Order</h5>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select" disabled>
                                        <option selected>{{ $radiologyType->name ?? 'Radiology' }}</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="patient_id" id="patient_id" class="form-select select2-in-modal"
                                        data-placeholder="-- Select Patient --" required>
                                        <option value=""></option>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="doctor_id" id="doctor_id" class="form-select select2-in-modal"
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Initialise Select2 on patient/doctor when the create modal opens.
            // dropdownParent must be the modal element or the open dropdown
            // loses focus to Bootstrap's modal focus trap.
            const createModalEl = document.getElementById('createModal');
            if (createModalEl && window.jQuery && typeof jQuery.fn.select2 === 'function') {
                createModalEl.addEventListener('shown.bs.modal', function () {
                    jQuery('#createModal .select2-in-modal').each(function () {
                        if (jQuery(this).hasClass('select2-hidden-accessible')) return;
                        jQuery(this).select2({
                            dropdownParent: jQuery('#createModal'),
                            width: '100%',
                ...
                createModalEl.addEventListener('hidden.bs.modal', function () {
                    jQuery('#createModal .select2-in-modal').val(null).trigger('change');
                });
            }
            ...
            const tbody = document.getElementById('radiologyTbody');
            const rowsSel = document.getElementById('rowsPerPage');
            const pagination = document.getElementById('pagination');
            const searchEl = document.getElementById('radiologySearch');
            let currentPage = 1;
...
@push('styles')
    <style>
        /* The <form> inside .modal-content breaks Bootstrap's flex chain for
           modal-dialog-scrollable. display: contents removes the form from the
           visual tree so header/body/footer become direct children of
           .modal-content, and Bootstrap's scrollable math works correctly. */
        #createModal .modal-content > form { display: contents !important; }

        /* Belt and suspenders: cap the dialog to viewport and force the body
           to scroll. Works even if a downstream stylesheet overrides flex. */
        ...
        #createModal .modal-body {
            max-height: calc(100vh - 3.5rem - 130px) !important;
