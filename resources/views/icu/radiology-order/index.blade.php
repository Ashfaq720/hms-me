...
@section('title', 'Radiology Orders — ' . $admission->icu_case_id)
    ...
    <div class="container">
            ...
            <div class="d-flex flex-wrap gap-2">
                @if (in_array($admission->status, ['Approved', 'Admitted']))
                    <a data-size="lg" class="btn btn-primary btn-sm"
                        data-url="{{ route('icu.admissions.radiology-orders.create', $admission->id) }}"
                        data-ajax-popup="true" data-title="Add Radiology Order"
                        data-bs-toggle="tooltip" title="Add Radiology Order"
                        href="#">
                        <i class="bi bi-plus-lg"></i> Radiology Order
                    </a>
                @endif
                <a href="{{ route('icu.admissions.show', $admission->id) }}"
        ...
        <div class="card mt-2">
            <div class="card-header py-2">
                <h6 class="mb-0 fw-semibold">Radiology Queue</h6>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-bordered align-middle mb-0">
                        ...
                        <tbody>
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li>
                                                    <a class="dropdown-item"
                                                        href="{{ route('icu.admissions.radiology-orders.show', [$admission->id, $order->id]) }}">
                                                ...
                                                @if (in_array($admission->status, ['Approved', 'Admitted']))
                                                    <li>
                                                        <a class="dropdown-item" data-size="lg"
                                                            data-url="{{ route('icu.admissions.radiology-orders.edit', [$admission->id, $order->id]) }}"
                                                    ...
                                                    <li>
                                                        <form
                                                            action="{{ route('icu.admissions.radiology-orders.destroy', [$admission->id, $order->id]) }}"
                                                            method="POST"
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="text-center text-muted py-4">No radiology orders yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
