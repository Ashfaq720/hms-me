<form action="{{ route('icu.admissions.medicine-orders.store', $admission->id) }}" method="POST">
    @csrf
                    ...
                    <td>
                        <select name="orders[0][prescribed_by]" class="form-select form-select-sm">
                            ...
                            @foreach ($doctors as $doctor)
                                <option value="{{ $doctor->id }}" @selected($admission->referring_doctor_id == $doctor->id)>{{ $doctor->name }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
