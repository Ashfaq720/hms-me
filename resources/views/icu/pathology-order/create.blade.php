<form action="{{ route('icu.admissions.pathology-orders.store', $admission->id) }}" method="POST">
    @csrf
        ...
        <div class="col-md-6">
            ...
            <select name="doctor_id" id="doctor_id"
                ...
                @foreach ($doctors as $doctor)
                    <option value="{{ $doctor->id }}" @selected(old('doctor_id', $admission->referring_doctor_id) == $doctor->id)>{{ $doctor->name }}</option>
                @endforeach
            </select>
            @error('doctor_id')
