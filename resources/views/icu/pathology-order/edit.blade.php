<form action="{{ route('icu.admissions.pathology-orders.update', [$admission->id, $order->id]) }}" method="POST">
    @csrf
    @method('PUT')

    @php($pathology = $investigationTypes->first())
    <input type="hidden" name="lab_inv_type_id" value="{{ $pathology->id }}">

    <div class="row g-3">

        {{-- Investigation Type (locked to Pathology) --}}
        <div class="col-md-6">
            <label class="form-label">Investigation Type</label>
            <select class="form-select" disabled>
                <option selected>{{ $pathology->name }}</option>
            </select>
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="datetime-local" name="datetime" id="datetime"
                ...
                value="{{ old('datetime', optional($order->datetime)->format('Y-m-d\TH:i')) }}">
            @error('datetime')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="lab_name" id="lab_name"
                ...
                value="{{ old('lab_name', $order->lab_name) }}" placeholder="Enter lab name">
            @error('lab_name')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="collected_by" id="collected_by"
                ...
                value="{{ old('collected_by', $order->collected_by) }}" placeholder="Enter collector name">
            @error('collected_by')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="col-md-6">
            ...
            @php($currentPriority = old('priority', $order->priority ?? 'Regular'))
            <select name="priority" id="priority"
                class="form-select @error('priority') is-invalid @enderror">
                <option value="Regular" @selected($currentPriority === 'Regular')>Regular</option>
                <option value="Urgent" @selected($currentPriority === 'Urgent')>Urgent</option>
                <option value="STAT" @selected($currentPriority === 'STAT')>STAT</option>
            </select>
            @error('priority')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        <div class="col-12">
            ...
            <textarea name="remarks" id="remarks" rows="2"
                ...
                placeholder="Enter remarks">{{ old('remarks', $order->remarks) }}</textarea>
            @error('remarks')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
...
(function () {
    ...
    const existingRequests = @json(old('requests', $order->requests->map(fn($r) => ['lab_inv_category_id' => $r->lab_inv_category_id, 'lab_inv' => $r->lab_inv_id])->values()));
    ...
    if (Array.isArray(existingRequests) && existingRequests.length) {
        existingRequests.forEach(r => buildRow(r.lab_inv_category_id || '', r.lab_inv || ''));
    } else {
        buildRow();
    }
})();
