...
@section('title', 'Transfer — ' . $admission->icu_case_id)
        ...
        <form method="POST" action="{{ route('icu.admissions.transfer.store', $admission->id) }}" class="mt-2">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row g-2">
                        @php
                            $currentUnit = $admission->icu_type ?: 'ICU';
                            $isCcu = $currentUnit === 'CCU';
                        @endphp
                        <div class="col-md-3">
                            ...
                            <select name="transfer_type" id="transfer_type" class="form-select" required>
                                <option value="IcuToWard">{{ $currentUnit }} → IPD</option>
                                <option value="IcuToCcu">{{ $currentUnit }} → CCU{{ $isCcu ? ' (different bed)' : '' }}</option>
                                <option value="IcuToIcu">{{ $currentUnit }} → ICU{{ $isCcu ? '' : ' (different bed)' }}</option>
                            </select>
                        </div>
                        <div class="col-md-3">
