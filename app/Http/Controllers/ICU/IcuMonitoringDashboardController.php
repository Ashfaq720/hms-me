...
class IcuMonitoringDashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Ventilators currently in use (per-admission counts)
        $ventilatorUsage = IcuEquipmentUsageLog::whereIn('icu_admission_id', $admissions->pluck('id'))
            ->where('equipment_type', 'Ventilator')
            ->where('status', 'InUse')
            ->selectRaw('icu_admission_id, COUNT(*) as cnt')
            ->groupBy('icu_admission_id')
            ->pluck('cnt', 'icu_admission_id');
        ...
        return view('icu.dashboard.index', compact(
            'admissions', 'latestVitals', 'openAlerts', 'activeCodes', 'ventilatorUsage',
