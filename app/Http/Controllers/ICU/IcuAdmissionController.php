...
class IcuAdmissionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // KPI stats are computed over currently active patients in the (optionally) filtered unit.
        $activeAdmissions = $admissions->whereIn('status', ['Admitted', 'Approved']);
        $totalActive      = $activeAdmissions->count();

        $isolationCount = $activeAdmissions->filter(function ($a) {
            return ! empty($a->isolation_type) && $a->isolation_type !== 'None';
        })->count();

        $ventilatorCount = $activeAdmissions->where('ventilator_required', true)->count();

        $criticalCount = $totalActive > 0
            ? IcuAlert::whereIn('icu_admission_id', $activeAdmissions->pluck('id'))
                ->whereIn('status', ['Active', 'Acknowledged'])
                ->where('severity', 'Critical')
                ->distinct('icu_admission_id')
                ->count('icu_admission_id')
            : 0;

        return view('icu.admissions.index', compact(
            'admissions', 'totalActive', 'isolationCount', 'ventilatorCount', 'criticalCount'
        ));
    }
    ...
    public function create(Request $request)
    {
        ...
        $patients = Patient::select('id', 'patient_name', 'mrn', 'mobileno', 'gender', 'dob', 'blood_group', 'known_allergies')
            ->get();
