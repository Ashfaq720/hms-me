...
class HealthCardController extends Controller
{
    ...
    public function checkin(Request $request, CaseReferenceService $caseService)
    {
        $request->validate([
        ...
        switch ($destination) {
            ...
            case 'opd':
                $bookingStatus = $request->input('booking_status', 'WALK_IN');

                $request->validate([
                    'department_id'   => 'required|exists:departments,id',
                    'doctor_id'       => 'required|exists:doctors,id',
                    'visit_type'      => 'required|in:new,follow_up,recheckup,referred,emergency',
                    'booking_status'  => 'nullable|in:PRE_BOOK,WALK_IN,REFERRAL',
                    'chief_complaint' => 'nullable|string|max:1000',
                ]);

                if ($bookingStatus === 'PRE_BOOK') {
                    $request->validate([
                ...
                $record = DB::transaction(function () use ($request, $patientId, $date, $bookingStatus, $caseService) {
                    $caseId     = $caseService->createCase($patientId, 'OPD');
                    $department = Department::find($request->input('department_id'));

                    $slotFrom = $slotTo = null;
                    if ($bookingStatus === 'PRE_BOOK' && $request->filled('slot_time')) {
                        [$slotFrom, $slotTo] = array_pad(explode('|', $request->input('slot_time'), 2), 2, null);
                    }
                    ...
                    $opd = OpdPatient::create([
                        ...
                        'shift_id'        => $bookingStatus === 'PRE_BOOK' ? $request->input('shift_id') : null,
                    ...
                    if ($bookingStatus === 'PRE_BOOK') {
                        Appointment::create([
                            ...
                            'amount'             => 0,
                            'message'            => $request->input('chief_complaint'),
            ...
            case 'er':
                ...
                ErPatient::create([
                    'patient_id'   => $patientId,
                    'case_id'      => $caseService->createCase($patientId, 'ER'),
                    'arrival_time' => $date,
            ...
            case 'ipd':
                $allocChoice = $request->input('allocation_choice', 'bed');
                ...
                $request->validate([
                    ...
                    'bed_id'                  => $allocChoice === 'icu' ? 'nullable|exists:beds,id' : 'required|exists:beds,id',
                    'icu_bed_id'              => $allocChoice === 'icu' ? 'required|exists:beds,id' : 'nullable|exists:beds,id',
                    'from'                    => 'nullable|date',
                ...
                return DB::transaction(function () use ($request, $patientId, $date, $allocChoice, $caseService) {
                    $caseId        = $caseService->createCase($patientId, 'IPD');
                    // A) IPD record
                    ...
                    $allocatedBedId   = $allocChoice === 'icu'
    ...
    public function labInvestigations(Request $request)
    {
        ...
        $names = match ($type) {
            'pathology' => ['Pathology'],
            'radiology' => ['Radiology'],
            default     => ['Pathology', 'Radiology'],
        };

        foreach ($names as $typeName) {
            $labType = LabInvestigationType::where('name', $typeName)->first();
            if (! $labType) continue;

            $categories = LabInvestigationCategory::where('type_id', $labType->id)
                ->orderBy('name')
                ->get()
                ->map(function ($cat) {
                    $investigations = LabInvestigation::where('category_id', $cat->id)
                        ->where('status', true)
                        ->orderBy('name')
                        ->get(['id', 'name', 'price']);
                ...
                ->filter(fn ($cat) => count($cat['investigations']) > 0)
                ->values();
