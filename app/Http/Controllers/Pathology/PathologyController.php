...
class PathologyController extends Controller
{
    ...
    {
        // ?type= scopes the page to one investigation type (e.g. Histopathology,
        // Microbiology, Endocrinology). With no query string we show every
        // order — that's the "All Orders" entry in the sidebar.
        $typeName = trim((string) $request->query('type', ''));

        $ordersQuery = LabInvestigationOrder::with([
                'patient',
                'doctor',
                'generatedBy',
        ...
        if ($typeName !== '') {
            $ordersQuery->where('type', $typeName);
        }
        ...
        // For the create modal we need a concrete type. If the page is filtered
        // by ?type= we use that one; otherwise fall back to Pathology so the
        // "All Orders" view still lets the user create something sensible.
        $pathologyType  = LabInvestigationType::where('name', $typeName !== '' ? $typeName : 'Pathology')
        ...
        return view('pathology.index', compact(
            'orders', 'patients', 'doctors', 'pathologyType', 'categories', 'investigations', 'typeName'
        ));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            DB::transaction(function () use ($validated) {
                ...
                // Derive the type label from the selected investigation type, so
                // orders created from /pathology?type=Histopathology are stored
                // with type='Histopathology' (and the listing filters correctly).
                $typeName = LabInvestigationType::where('id', $validated['lab_inv_type_id'])
                    ->value('name') ?? 'Pathology';
