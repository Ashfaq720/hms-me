...
class IpdPatientController extends Controller
{
    ...
    public function store(IPDPatientRequest $request)
    {
        ...
        try {
            ...
            // Optional package enrolment — auto-post bundle services to the bill
            if ($request->filled('enroll_package_id')) {
                try {
                    $pkg = \App\Models\Package::find($request->input('enroll_package_id'));
                    if ($pkg && $pkg->is_active) {
                        $encounterId = $ipd->encounter_id;
                        $duration = (int) ($request->input('enroll_duration_days') ?: ($pkg->validity_days ?: 1));
                        if ($encounterId) {
                            \App\Models\Package\PackageEnrollment::create([
                                'package_id'   => $pkg->id,
                                'encounter_id' => $encounterId,
                                'patient_id'   => $patient->id,
                                'enrollment_no' => 'PE-' . now()->format('YmdHis') . '-' . $patient->id,
                                'start_date'   => now(),
                                'end_date'     => now()->addDays($duration),
                                'agreed_price' => $pkg->total_amount,
                                'paid_amount'  => 0,
                                'status'       => 'active',
                                'notes'        => 'Auto-enrolled at IPD admission',
                                'created_by'   => auth()->id(),
                            ]);
                        }
                    }
                } catch (\Throwable $pe) {
                    Log::warning('Package auto-enroll on IPD admission failed', ['err' => $pe->getMessage()]);
                }
            }
    ...
    public function show(IpdPatient $iPDPatient)
    {
        $iPDPatient->load([
            ...
            'bedAllocations.bed.bedType', 'bedAllocations.bed.bedGroup',
            'nurseNotes', 'charges', 'transactions',
            'prescriptions.medicines.medicine',
            'medicineOrders.medicine', 'medicineOrders.prescribedBy',
            'documents', 'surgeryRequests.primarySurgeon',
        ]);

        // Pull dynamic charges + bill data from the unified encounter layer
        // so every tab on the show page can render live values.
        $encounterId = $iPDPatient->encounter_id;
        $encounter   = $encounterId ? \App\Models\Encounter\Encounter::with([
            'serviceChargePostings.catalog',
        ])->find($encounterId) : null;

        $chargePostings = $encounter ? $encounter->serviceChargePostings : collect();
        $chargeTotal    = (float) $chargePostings->where('status', 'posted')->sum('net_amount');
        $chargeCount    = $chargePostings->where('status', 'posted')->count();

        $bills          = $encounterId ? \App\Models\Billing\Bill::with(['items.service','payments','refunds','discounts'])
                            ->where('encounter_id', $encounterId)
                            ->latest('id')
                            ->get() : collect();
        $primaryBill    = $bills->first();

        $insurancePolicy = \App\Models\Insurance\InsurancePolicy::where('patient_id', $iPDPatient->patient_id)
            ->where('status', 'active')
            ->latest('id')->first();
        $claims          = $encounterId ? \App\Models\Insurance\Claim::where('encounter_id', $encounterId)->get() : collect();

        return view('ipd_patients.show', compact(
            'iPDPatient',
            'encounter', 'chargePostings', 'chargeTotal', 'chargeCount',
            'bills', 'primaryBill',
    ...
    /**
     * Show transfer-to-acuity form. Accepts optional ?type=ICU|NICU|CCU
     * so the modal can be opened pre-filtered from the IPD list.
     */
    public function icuTransfer($id, Request $request = null)
    {
        $id   = (int) $id;
        ...
        $icuBeds = Bed::with('bedType')
            ->where('is_reserved', false)
            ->get()
            ->filter(function ($b) use ($type) {
                if (! optional($b->bedType)->is_icu) {
                    return false;
                }
                if ($type === '') {
                    return true;
                }
                return strtoupper((string) $b->bedType->icu_type) === $type
                    || strtoupper((string) $b->bedType->name) === $type;
            })
            ->values();

        $title = match ($type) {
            'NICU' => 'Transfer to NICU',
            'CCU'  => 'Transfer to CCU',
            'ICU'  => 'Transfer to ICU',
            default => 'Transfer to ICU / CCU / NICU',
        };

        return view('ipd_patients.icutransfer', compact('icuBeds', 'id', 'type', 'title'));
    }
    ...
    protected function performAllocationTransfer(Request $request, $id, string $direction)
    {
        ...
        try {
            ...
            $allocationType = 'bed';
            if ($direction === 'icu') {
                $acuity = strtoupper((string) (optional($newBed->bedType)->icu_type ?: $newBed->bedType?->name));
                $allocationType = match ($acuity) {
                    'NICU' => 'nicu',
                    'CCU'  => 'ccu',
                    'ICU'  => 'icu',
                    default => 'icu',
    ...
    /**
     * Show the enrol-package modal for an IPD patient. Loads packages
     * relevant to IPD-type flows (IPD / OT / ICU / MATERNITY).
     */
    public function enrollPackageForm($id)
    {
        $id = (int) $id;
        ...
        return view('ipd_patients.enroll_package', compact('ipdPatient', 'packages', 'id'));
    }

    /**
     * Create a PackageEnrollment + per-service consumption entries against
     * the IPD encounter. Idempotent on (patient_id, package_id, encounter_id).
     */
    public function enrollPackageStore(Request $request, $id)
    {
        $id = (int) $id;
        $request->validate([
        ...
        try {
            $ipdPatient = IpdPatient::findOrFail($id);
            $package = \App\Models\Package::with('services')->findOrFail($request->package_id);

            $enrollment = \App\Models\Package\PackageEnrollment::firstOrCreate(
                [
                    'patient_id' => $ipdPatient->patient_id,
                    'package_id' => $package->id,
                    'encounter_id' => $ipdPatient->encounter_id,
                ],
                [
                    'start_date' => $request->start_date,
                    'end_date' => $request->end_date ?: Carbon::parse($request->start_date)->addDays($package->validity_days ?: 30),
                    'agreed_price' => $request->agreed_price ?? $package->total_amount,
            ...
            foreach ($package->services as $ps) {
                \App\Models\Package\PackageConsumptionEntry::firstOrCreate(
                    ['package_enrollment_id' => $enrollment->id, 'package_service_id' => $ps->id],
                    [
                        'description' => $ps->service->name ?? 'Package service',
            ...
            DB::commit();
            return redirect()->route('ipd-patients.show', $ipdPatient->id)
                ->with('success', 'Package "' . $package->name . '" enrolled successfully.');
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('Package enrollment failed', ['message' => $e->getMessage()]);
            return redirect()->back()->with('error', 'Package enrolment failed: ' . $e->getMessage());
        }
    }

    public function enrollPackageDestroy($id, $enrollmentId)
    {
        $enrollment = \App\Models\Package\PackageEnrollment::findOrFail($enrollmentId);
        $enrollment->update(['status' => 'cancelled']);
        return redirect()->route('ipd-patients.show', $id)
            ->with('success', 'Package enrolment cancelled.');
    }
}
