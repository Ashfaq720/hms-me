...
class PathologyOrderController extends Controller
{
    protected string $orderType = 'pathology';

    public function create($id)
    {
        $ipdPatient         = IpdPatient::with('patient')->findOrFail($id);
        $doctors            = Doctor::select('id', 'name')->orderBy('name')->get();
        $pathologyType      = LabInvestigationType::where('name', 'Pathology')->select('id', 'name')->firstOrFail();
        $investigationTypes = collect([$pathologyType]);
        $categories         = LabInvestigationCategory::where('type_id', $pathologyType->id)
            ->select('id', 'type_id', 'name')->orderBy('name')->get();
        $investigations = LabInvestigation::whereIn('category_id', $categories->pluck('id'))
            ->select('id', 'category_id', 'name')->orderBy('name')->get();

        return view('ipd_patients.pathology-order.create', compact(
            'id',
            'ipdPatient',
            'doctors',
            'investigationTypes',
    ...
    public function store(Request $request, $id)
    {
        ...
        try {
            ...
            //     ->with('success', 'Pathology order saved successfully.');

            return redirect(route('ipd-patients.ipd-patients.show', $id) . '?tab=lab')
                ->with('success', 'Pathology order saved successfully.');
        } catch (\Exception $e) {
            dd($e->getMessage());
    ...
    public function show($id, $orderId)
    {
        ...
        return view('ipd_patients.pathology-order.show', compact('ipdPatient', 'order'));
    }
    ...
    public function edit($id, $orderId)
    {
        ...
        return view('ipd_patients.pathology-order.edit', compact(
            'id',
            'ipdPatient',
            'order',
            'doctors',
    ...
    public function update(Request $request, $id, $orderId)
    {
        ...
        try {
            ...
            // return redirect()->route('ipd-patients.ipd-patients.show', $id)
            //     ->with('success', 'Pathology order updated successfully.');

            return redirect(route('ipd-patients.ipd-patients.show', $id) . '?tab=lab')
                ->with('success', 'Pathology order updated successfully.');
        } catch (\Exception $e) {
            return redirect()->back()->withInput()
                ->with('error', 'Failed to update pathology order: ' . $e->getMessage());
        }
    }
    ...
    public function destroy($id, $orderId)
    {
        try {
            ...
            // return redirect()->route('ipd-patients.ipd-patients.show', $id)
            //     ->with('success', 'Pathology order deleted successfully.');

            return redirect(route('ipd-patients.ipd-patients.show', $id) . '?tab=lab')
                ->with('success', 'Pathology order deleted successfully.');
        } catch (\Exception $e) {
            return redirect()->back()
                ->with('error', 'Failed to delete pathology order: ' . $e->getMessage());
        }
    }
