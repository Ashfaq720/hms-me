...
class IpdPatientController extends Controller
{
    ...
    public function icuTransfer($id)
    {
        ...
        return view('ipd_patients.icutransfer', compact('icuBeds', 'id', 'ipdPatient', 'doctors'));
    }
    ...
    public function icuTransferStore(Request $request, $id)
    {
        ...
        $data = $request->validate([
            'icu_type'                  => ['required', \Illuminate\Validation\Rule::in(['ICU', 'CCU', 'NICU', 'PICU'])],
            'isolation_type'            => ['nullable', \Illuminate\Validation\Rule::in(['None', 'Airborne', 'Contact', 'Droplet', 'Standard'])],
            'admission_type'            => ['nullable', \Illuminate\Validation\Rule::in(['Emergency', 'Planned', 'Transfer'])],
            'admission_diagnosis'       => ['required', 'string', 'max:1000'],
            'referring_doctor_id'       => ['required', 'integer', \Illuminate\Validation\Rule::exists('doctors', 'id')],
            'ventilator_required'       => ['nullable', 'boolean'],
            'monitor_required'          => ['nullable', 'boolean'],
            'icu_bed_id'                => ['nullable', 'integer', \Illuminate\Validation\Rule::exists('beds', 'id')],
            'from'                      => ['required', 'date'],
            'remarks'                   => ['nullable', 'string', 'max:2000'],
            // Emergency Override
            'override'                  => ['nullable', 'boolean'],
            'override_resource_issue'   => ['nullable', \Illuminate\Validation\Rule::in(['NoBed', 'NoVentilator', 'NoMonitor', 'NoIsolationBed', 'Other'])],
            'override_approved_by'      => ['nullable', 'integer'],
            'override_temporary_bed_id' => ['nullable', 'integer', \Illuminate\Validation\Rule::exists('beds', 'id')],
            'override_reason'           => ['nullable', 'string', 'max:1000'],
        ]);

        $useOverride = (bool) ($data['override'] ?? false);
        $bedId       = $data['icu_bed_id'] ?? null;

        DB::beginTransaction();
        try {
            if (! $useOverride) {
                $this->validateIcuBedPolicy([
                    'icu_type'            => $data['icu_type'],
                    'isolation_type'      => $data['isolation_type'] ?? 'None',
                    'ventilator_required' => (bool) ($data['ventilator_required'] ?? false),
                ], $bedId);
            } else {
            ...
            // Close any open Ipd allocation + post charge for prior bed-stay
            $current = $ipdPatient->bedAllocations()->latest('id')->first();
            if ($current) {
                Bed::where('id', $current->bed_id)->update(['is_reserved' => false]);
                $current->update([
                ...
                $from      = Carbon::parse($current->from);
                $to        = Carbon::parse($current->to);
                $totalDays = $from->copy()->startOfDay()->diffInDays($to->copy()->startOfDay());
                if ($to->format('H:i:s') > '12:00:00') {
                    $totalDays++;
                }
                $totalDays = max($totalDays, 1);
            ...
            // Mirror ICU allocation in Ipd timeline
            IpdPatientBed::create([
                'case_id'         => $ipdPatient->case_id,
                'ipd_patient_id'  => $ipdPatient->id,
                'bed_id'          => $bedId,
            ...
            $admission = IcuAdmission::create([
                ...
                'source_id'           => $ipdPatient->id,
                'icu_type'            => $data['icu_type'],
                'admission_type'      => $data['admission_type'] ?? 'Transfer',
                'admission_diagnosis' => $data['admission_diagnosis'],
                'referring_doctor_id' => $data['referring_doctor_id'],
                'isolation_type'      => $data['isolation_type'] ?? 'None',
                'ventilator_required' => (bool) ($data['ventilator_required'] ?? false),
                'monitor_required'    => (bool) ($data['monitor_required'] ?? true),
                'bed_id'              => $bedId,
                'admission_time'      => $data['from'],
                'status'              => 'Admitted',
                'remarks'             => $data['remarks'] ?? null,
                'created_by'          => auth()->id(),
            ...
            if ($useOverride) {
                \App\Models\Icu\IcuAdmissionOverride::create([
                ...
                if (
                    ...
                    \App\Models\Icu\IcuInfectionControlOverride::create([
                        'icu_admission_id'        => $admission->id,
            ...
            DB::commit();
            return redirect()->back()->with('success', "Patient transferred to {$data['icu_type']}. Case ID: {$icuCaseId}");
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    /**
     * Mirror IcuAdmissionController::validateResources — checks the selected
     * ICU bed against the requested icu_type / isolation_type / ventilator.
     */
    protected function validateIcuBedPolicy(array $data, ?int $bedId): void
    {
        if (! $bedId) {
