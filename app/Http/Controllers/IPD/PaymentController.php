...
class PaymentController extends Controller
{
    ...
    {
        $ipdPatient = IpdPatient::with(['patient', 'doctor', 'department', 'transactions', 'charges'])->findOrFail($ipdPatientId);

        // Pull the encounter-layer financials so the payment form pre-fills the
        // real outstanding amount instead of starting from zero.
        $bills = collect();
        $encounterCharges = collect();
        $packageEnrollments = collect();
        if ($ipdPatient->encounter_id) {
            $bills = \App\Models\Billing\Bill::with(['items', 'payments'])
            ...
            $packageEnrollments = \App\Models\Package\PackageEnrollment::with('package')
        ...
        $totals = [
            'legacy_charges'      => (float) $ipdPatient->charges->sum('net_amount'),
            'legacy_paid'         => (float) $ipdPatient->transactions->sum('net_amount'),
            'encounter_charges'   => (float) $encounterCharges->sum('net_amount'),
            ...
            'package_agreed'      => (float) $packageEnrollments->sum('agreed_price'),
            'package_paid'        => (float) $packageEnrollments->sum('paid_amount'),
        ];
        $totals['package_outstanding'] = max($totals['package_agreed'] - $totals['package_paid'], 0);
        // Suggested amount = bill balance + package outstanding (single number to pay)
        $totals['suggested_amount']    = $totals['bill_balance_due'] + $totals['package_outstanding'];

        return view('ipd_patients.payments.create', compact(
            'ipdPatient', 'bills', 'encounterCharges', 'packageEnrollments', 'totals'
        ));
    }
