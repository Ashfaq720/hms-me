...
class FrontDeskController extends Controller
{
    ...
    public function er_registration()
    {
        ...
        return view('front-desk.er-registration.create', compact('departments', 'doctors'));
    }
    ...
    public function er_registration_store(Request $request, CaseReferenceService $caseService)
    {
        ...
        $data = $request->validate([
            ...
            'name'          => $isNew ? 'required|string|max:255' : 'nullable|string|max:255',
        ...
        try {
            if (! $isNew && ! empty($data['patient_id'])) {
                ...
                $patient                = new Patient();
                $patient->patient_name  = $data['name'] ?? 'Unknown';
                $patient->mobileno      = $data['contact_no'];
            ...
            $caseId = $caseService->createCase($patient->id, 'ER');
            ...
            return back()->with('success', 'ER patient registered successfully.');
        } catch (\Throwable $e) {
            DB::rollBack();
