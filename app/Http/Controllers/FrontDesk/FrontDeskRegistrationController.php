...
class FrontDeskRegistrationController extends Controller
{
    public function create()
    {
        ...
        $beds = Bed::select('id', 'name', 'rent', 'bed_type_id', 'bed_group_id')
            ->with(['bedType', 'bedGroup'])
            ->where('is_reserved', 0)
            ->where(function ($q) {
                $q->whereDoesntHave('bedType')
                  ->orWhereHas('bedType', fn($t) => $t->where('is_icu', 0));
            })
            ->get();

        $icuBeds = Bed::select('id', 'name', 'rent', 'bed_type_id')
            ->with(['bedType'])
            ->where('is_reserved', 0)
            ->whereHas('bedType', fn($q) => $q->where('is_icu', 1))
            ->get();

        return view('front-desk.registration.create', compact('doctors', 'departments', 'organizations', 'patients', 'beds', 'icuBeds'));
    }
        ...
        try {
            ...
            if ($data['patient_type'] === 'Ipd') {
                $ipd_patient = IpdPatient::create([
                    ...
                    'admission_date'          => $data['ipd_admission_date'] ?? now(),
                    'possible_discharge_date' => $data['possible_discharge_date'] ?? null,
                    'admission_type'          => $data['admission_type'] ?? null,
                    'patient_history'         => $data['patient_history'] ?? null,
                    'remarks'                 => $data['ipd_remarks'] ?? $data['description'] ?? null,
                    'status'                  => $data['ipd_status'] ?? 'Admitted',
                ]);

                // Determine which bed was selected (regular vs ICU)
                $isIcu  = ($data['allocation_choice'] ?? 'bed') === 'icu';
                $bedId  = $isIcu
                    ? ($data['icu_bed_id'] ?? null)
                    : ($data['bed_id']     ?? null);
                ...
                if ($bedId) {
                    IpdPatientBed::create([
                        ...
                        'bed_id'          => $bedId,
                        'allocation_type' => $isIcu ? 'icu' : 'bed',
                        'from'            => $data['from'] ?? now(),
                        'to'              => $data['to'] ?? null,
                        'remarks'         => $data['bed_remarks'] ?? null,
                    ]);

                    Bed::where('id', $bedId)->update(['is_reserved' => 1]);
                }
            }
