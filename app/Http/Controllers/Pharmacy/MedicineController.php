...
class MedicineController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Create the medicine row + its inventory_items twin in one transaction
        // so every new medicine immediately participates in the unified
        // inventory ledger (no need to re-run the unification seeder).
        DB::transaction(function () use ($request, $photoName) {
            $categoryName = MedicineCategory::where('id', $request->medicine_category_id)->value('name');
            $unitName     = MedicineUnit::where('id', $request->medicine_unit_id)->value('name');
            $brandName    = $request->company_id
                ? Company::where('id', $request->company_id)->value('name')
                : null;
            ...
            // Build the canonical inventory_items twin. Code matches the
            // seeder's MED-NNNNN pattern so old and new entries are
            // indistinguishable.
            $code = 'MED-' . str_pad((string) $medicine->id, 5, '0', STR_PAD_LEFT);
            $item = InventoryItem::create([
                'organization_id'   => DB::table('organizations')->value('id'),
                'code'              => $code,
                'name'              => $medicine->medicine_name,
                'category'          => $categoryName ?: 'Medicine',
                'generic_name'      => $medicine->medicine_composition,
                'brand'             => $brandName,
                'sku'               => $code,
                'uom'               => $unitName ?: 'piece',
                'tax_percent'       => (float) ($medicine->tax ?? 0),
                'reorder_level'     => is_numeric($medicine->reorder_level) ? (float) $medicine->reorder_level : 0,
                'reorder_quantity'  => 0,
                'storage_condition' => $medicine->rack_number ? 'Rack: ' . $medicine->rack_number : null,
                'is_controlled'     => 0,
                'is_consumable'     => 1,
                'is_asset'          => 0,
                'is_active'         => $medicine->status ? 1 : 0,
                'description'       => $medicine->medicine_composition,
            ]);

            $medicine->update(['inventory_item_id' => $item->id]);
        });

        return redirect()->route('admin.pharmacy.drug-master')
            ->with('success', 'Medicine created successfully (with inventory link).');
    }
