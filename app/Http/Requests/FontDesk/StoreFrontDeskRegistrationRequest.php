...
class StoreFrontDeskRegistrationRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'admission_type'         => ['nullable', Rule::in(['Emergency', 'Planned', 'Unplanned', 'Transfer(Internal)', 'Transfer(External)'])],
            'ipd_status'             => ['nullable', Rule::in(['Admitted', 'Discharged', 'Transferred', 'Cancelled'])],
            'patient_history'        => ['nullable', 'string', 'max:2000'],
            'ipd_remarks'            => ['nullable', 'string', 'max:2000'],

            // ipd bed
            'allocation_choice'     => ['nullable', Rule::in(['bed', 'icu'])],
            'bed_id'                => ['nullable', 'integer', 'exists:beds,id'],
            'icu_bed_id'            => ['nullable', 'integer', 'exists:beds,id'],
            'from'                  => ['nullable', 'date'],
            'to'                    => ['nullable', 'date'],
    ...
    public function withValidator(\Illuminate\Validation\Validator $validator): void
    {
        $validator->after(function ($v) {
            ...
            if ($patType === 'OPD' && empty($data['booking_status'])) {
                $v->errors()->add('booking_status', 'Please select a booking status.');
            }

            if ($patType === 'Ipd' && empty($data['ipd_admission_date'])) {
                $v->errors()->add('ipd_admission_date', 'Admission date is required for IPD.');
            }
            ...
            if ($patType === 'Ipd') {
                ...
                if ($isIcu && empty($data['icu_bed_id'])) {
                    $v->errors()->add('icu_bed_id', 'Please assign an ICU bed for IPD admission.');
                } elseif (!$isIcu && empty($data['bed_id'])) {
