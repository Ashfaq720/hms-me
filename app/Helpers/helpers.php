...
if (!function_exists('setting')) {
    ...
    {
        try {
            // Cache the full active-settings map for 10 minutes — settings change rarely
            // but are read on EVERY request (sidebar logo, company name, etc.).
            $all = \Illuminate\Support\Facades\Cache::remember(
                'hms.settings.active',
                600,
                fn () => Setting::where('is_active', true)
                    ->pluck('value', 'key')
                    ->all()
            );
...
if (!function_exists('company_info')) {
    ...
    {
        try {
            // Cache for 10 minutes; bust whenever Setting::saved fires
            return \Illuminate\Support\Facades\Cache::remember(
                'hms.settings.company_info',
                600,
                fn () => \App\Models\Setting::getCompanyInfo()
            );
        } catch (\Exception $e) {
            return [];
        }
...
if (!function_exists('hms_cache_bust_settings')) {
    /**
     * Invalidate cached settings. Call this from the Setting model's
     * `saved` and `deleted` events, or from settings admin controllers.
     */
    function hms_cache_bust_settings(): void
    {
        \Illuminate\Support\Facades\Cache::forget('hms.settings.active');
        \Illuminate\Support\Facades\Cache::forget('hms.settings.company_info');
    }
}
