    ...
    protected static function boot()
    {
        ...
        static::creating(function ($model) {
            if (empty($model->schedule_no)) {
                // Walk only well-formed SS-NNNNNN rows. Past data corruption
                // (e.g. 'SS-0000-4' or 'OTS-…' prefixes from older code) used
                // to poison the simple "latest by id" lookup and produce
                // collisions on SS-000001.
                $maxNum = (int) static::withTrashed()
                    ->where('schedule_no', 'REGEXP', '^SS-[0-9]{6}$')
                    ->selectRaw('MAX(CAST(SUBSTRING(schedule_no, 4) AS UNSIGNED)) as n')
                    ->value('n');
                $next = $maxNum + 1;
