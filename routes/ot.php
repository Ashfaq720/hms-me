...
use App\Http\Controllers\OT\SurgeryRequestStatusController;
...
Route::prefix('ot')
    ...
    ->group(function () {
        ...
        //    CRUD lives in SurgeryRequestController; state transitions in SurgeryRequestStatusController.
        Route::resource('surgery-requests', SurgeryRequestController::class);

        Route::prefix('surgery-requests/{id}')
            ->controller(SurgeryRequestStatusController::class)
            ->group(function () {
                Route::post('submit',              'submit')->name('surgery-requests.submit');
                Route::post('review',              'review')->name('surgery-requests.review');
                Route::post('accept',              'accept')->name('surgery-requests.accept');
                Route::post('reject',              'reject')->name('surgery-requests.reject');
                Route::post('send-back',           'sendBack')->name('surgery-requests.send-back');
                Route::post('pending-info',        'pendingInfo')->name('surgery-requests.pending-info');
                Route::post('fast-track',          'fastTrack')->name('surgery-requests.fast-track');
                Route::post('junior-approve',      'juniorApprove')->name('surgery-requests.junior-approve');
                Route::post('consultant-approve',  'consultantApprove')->name('surgery-requests.consultant-approve');
                Route::post('move-to-scheduling',  'moveToScheduling')->name('surgery-requests.move-to-scheduling');
                Route::post('cancel',              'cancel')->name('surgery-requests.cancel');
            });
